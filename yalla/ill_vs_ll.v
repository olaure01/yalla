...
- destruct (i2a_inv c) as [lc Hc].
  ...
  + destruct B; inversion Himg as [Himg']. subst.
    apply inT_map, Hc. reflexivity.
  + apply inT_map_inv in Himg as [i Himg].
    destruct B; inversion Himg as [Himg']. subst.
...
- exists (ione :: nil, nil); intros B; split; intros Himg.
  + destruct B; inversion Himg. apply inT_eq.
...
- destruct IHA1 as [(l1, l1op) H1 H1op].
  ...
  + subst. exists (nil, map (fun '(x, y) => ilmap x (ivar y)) (list_prod l2 lN) ++ map ineg l2);
    ...
    * destruct B; inversion Himg as [[Himg1 Himg2]].
      ...
      -- subst. destruct B2; inversion Himg1 as [Hi].
         apply HlN in Hi.
         apply inT_sum_app. left.
         change (ilmap B1 (ivar i)) with ((fun '(x, y) => ilmap x (ivar y)) (B1, i)).
         apply inT_map, inT_prod; [ apply H2, bidual | apply Hi ].
      -- inversion Himg1 as [Hi]. rewrite bidual in Hi.
         apply inT_sum_app. right.
         apply inT_map, H2, Hi.
    * apply inT_app_sum in Himg as [Himg|Himg].
      -- apply inT_map_inv in Himg as [(B', i) <- [Hin Hi]%inT_prod_inv].
         cbn. rewrite bidual. f_equal; [f_equal | ].
         ++ apply HlN, Hi.
         ++ apply H2, Hin.
      -- apply inT_map_inv in Himg as [B' <- ->%H2].
  ...
  + subst. exists (nil, map (fun '(x, y) => ilpam x (ivar y)) (list_prod l1 lN) ++ map igen l1);
    ...
    * destruct B; inversion Himg as [[Himg1 Himg2]].
      -- subst. destruct B2; inversion Himg2 as [Hi].
         ...
         apply inT_map, inT_prod; [ apply H1, bidual | apply Hi ].
      -- inversion Himg1 as [Hi]. rewrite bidual in Hi.
         apply inT_sum_app. right.
         apply inT_map, H1, Hi.
      -- destruct B1; discriminate Himg2.
    ...
    * apply inT_app_sum in Himg as [Himg|Himg].
      -- apply inT_map_inv in Himg as [(B', i) <- [Hin Hi]%inT_prod_inv].
         ...
         ++ apply H1, Hin.
         ++ apply HlN, Hi.
      -- apply inT_map_inv in Himg as [B' <- ->%H1].
         simpl ill2ll. simpl dual. rewrite bidual. reflexivity.
  + exists (map (fun '(x, y) => itens x y) (list_prod l1 l2),
    ...
    * destruct B; inversion Himg as [[Himg1 Himg2]].
      ...
      apply inT_map, inT_prod.
      -- apply H1. assumption.
      -- apply H2. assumption.
    * apply inT_map_inv in Himg as [(B1, B2) <- [Hin1 Hin2]%inT_prod_inv].
      cbn. f_equal.
      -- apply H1. assumption.
      -- apply H2. assumption.
    * destruct B; inversion Himg as [[Himg1 Himg2]].
      ...
      -- contradiction.
    * apply inT_app_sum in Himg as [Hin|Hin].
      -- apply inT_map_inv in Hin as [(B1, B2) <- [Hin1 Hin2]%inT_prod_inv].
         cbn. f_equal.
  ...
  + subst. exists (map (fun '(x, y) => ilpam x (ivar y)) (list_prod l2op lN) ++ map igen l2op, nil);
    ...
    * destruct B; inversion Himg as [[Himg1 Himg2]].
      -- subst. destruct B2; inversion Himg1 as [Hi].
         ...
         apply inT_sum_app. left.
         change (ilpam B1 (ivar i)) with ((fun '(x, y) => ilpam x (ivar y)) (B1, i)).
         apply inT_map, inT_prod; [ apply H2op; reflexivity | apply Hi ].
      -- inversion Himg1 as [Hi].
         apply inT_sum_app. right.
         apply inT_map, H2op, Hi.
      -- destruct B1; discriminate Himg1.
      -- contradiction n.
    * apply inT_app_sum in Himg as [Himg|Himg].
      -- apply inT_map_inv in Himg as [(B1, B2) <- [Hin Hi]%inT_prod_inv].
         cbn. f_equal; [ f_equal | ].
         ++ apply HlN, Hi.
         ++ apply H2op, Hin.
      -- apply inT_map_inv in Himg as [B' <- ->%H2op].
         reflexivity.
  ...
  + subst. exists (map (fun '(x, y) => ilmap x (ivar y)) (list_prod l1op lN) ++ map ineg l1op, nil);
    ...
    * destruct B; inversion Himg as [[Himg1 Himg2]].
      ...
      -- subst. destruct B2; inversion Himg2 as [Hi].
         ...
         apply inT_sum_app. left.
         change (ilmap B1 (ivar i)) with ((fun '(x, y) => ilmap x (ivar y)) (B1, i)).
         apply inT_map, inT_prod; [ apply H1op; reflexivity | apply Hi ].
      -- inversion Himg1 as [Hi].
         apply inT_sum_app. right.
         apply inT_map, H1op, Hi.
    * apply inT_app_sum in Himg as [Himg|Himg].
      -- apply inT_map_inv in Himg as [(B1, B2) <- [Hin Hi]%inT_prod_inv].
         cbn. f_equal; [ | f_equal ].
         ++ apply H1op, Hin.
         ++ apply HlN, Hi.
      -- apply inT_map_inv in Himg as [B' <- ->%H1op].
         reflexivity.
    * destruct B; inversion Himg as [[Himg1 Himg2]]. destruct B1; discriminate Himg2.
    * destruct Himg.
...
- destruct IHA as [(l1, l1op) H1 H1op].
  ...
  + destruct B; inversion Himg as [Himg1].
    apply inT_map, H1. assumption.
  + apply inT_map_inv in Himg as [B1 <- Hin].
    cbn. f_equal.
    apply H1. assumption.
  + destruct B; discriminate Himg.
  + destruct Himg.
...
- eapply ex_r, PCPermutationT_swap. apply ax_r.
- eapply ex_r; [ eassumption | ].
  apply PEPermutation_PCPermutationT_cons; [ reflexivity | ].
  apply PEPermutationT_map, PEPermutationT_map, PEPermutationT_rev, p.
- rewrite ? ill2ll_map_ioc, ? app_comm_cons in *.
  apply PermutationT_rev' in p. do 2 eapply PermutationT_map in p.
...
- apply parr_r.
  ...
  apply PCPermutationT_cons_append.
- rewrite app_comm_cons.
  eapply ex_r; [ | apply PCPermutationT_app_comm ].
...
- rewrite app_comm_cons.
  ...
  { cbn. destruct (ill2ll_inv (ill2ll A)) as [(l, lop) Hl _]. cbn.
    ...
    apply (exists_existsbT _ _ A); assumption. }
...
- symmetry in p.
  apply PEPermutationT_nil in p. now apply IHpi.
...
- symmetry in p. apply PEPermutationT_length_1_inv in p. now apply IHpi.
...
- apply parr_r.
  eapply ex_r; [ | symmetry; apply PermutationT_cons_append ].
  eapply ex_r; [ | symmetry; apply PermutationT_cons_append ].
  apply tens_r.
  + ll_swap. apply ax_r.
  + apply parr_r.
...
- symmetry in p.
  apply PEPermutationT_length_1_inv in p as ->. apply IHpi; reflexivity.
- destruct l1; inversion Heql; subst.
  + destruct lw'; inversion H0.
    cbn in H. subst.
    symmetry in p. apply PermutationT_nil in p as ->. apply IHpi; reflexivity.
...
induction pi in Heql, HeqC |- *; subst;
  ...
  try now (destruct l1; inversion Heql; destruct l1; discriminate).
- symmetry in p.
  apply PEPermutationT_length_1_inv in p. now apply IHpi.
- destruct l1; inversion Heql; subst.
  + destruct lw'; inversion H0.
    cbn in H. subst.
    symmetry in p. apply PermutationT_nil in p as ->. apply IHpi; reflexivity.
...
- apply IHpi; [ reflexivity | ].
  ...
  + destruct lw'; inversion H0.
    symmetry in p. apply PermutationT_nil in p as ->. assumption.
  + apply app_eq_nil in H1 as [-> [H1 ->]%app_eq_nil].
    destruct lw'; inversion H1.
    symmetry in p. apply PermutationT_nil in p as ->. reflexivity.
- destruct l1; inversion Heql; subst.
  + apply app_eq_nil in H2 as [-> ->]. clear - pi1.
    ...
    induction pi1 in Heql, HeqC |- *; subst;
      ...
      try now (destruct l1; inversion Heql; destruct l1; discriminate).
    * cbn in p. symmetry in p.
      apply PermutationT_length_1_inv in p as ->. apply IHpi1; reflexivity.
...
cut (forall l2, ll_mix0 (dual (ill2ll A) :: nil) ->
  PermutationT (oc F :: ill2ll A :: nil) l2 -> notT (ll_mix0 l2)).
...
- remember (covar (i2a i) :: nil) as l1. revert Heql1. clear - pi1.
  ...
  + destruct l1; inversion Heql1; destruct lw'; inversion H0.
    * symmetry in p. now apply PermutationT_nil in p as ->.
    * symmetry in p. now apply PermutationT_nil in p as ->.
    * destruct l1; discriminate H1.
  + destruct L; try inversion Heql1; discriminate f.
- revert HP2. clear - pi2. induction pi2; intros HP2;
    try (apply PermutationT_length in HP2; discriminate HP2);
    try now apply PermutationT_length_2_inv in HP2 as [ | ].
  + apply IHpi2.
  ...
  + destruct L; [ | discriminate f ].
    apply PermutationT_sym, PermutationT_nil_cons in HP2 as [].
  + apply PermutationT_length_2_inv in HP2.
  ...
  { revert HP2. clear - pi2; induction pi2; intros HP2;
    ...
    - apply PermutationT_length_2_inv in HP2.
      ...
      + destruct l1; inversion H2; subst.
        ...
        eapply ex_r; [ apply pi2_1 | apply PermutationT_swap ].
      + apply app_eq_nil in H1 as [-> ->].
        right. split; [ assumption | ].
        eapply ex_r; [ apply pi2_2 | apply PermutationT_swap ].
    - apply PermutationT_length_2_inv in HP2.
      destruct HP2; inversion e.
...
- eapply tens_rev in pi1; [ | intros [] | intro; reflexivity ].
  cons2app in HP2.
  assert (Heq2 := HP2).
  symmetry in Heq2; apply PermutationT_vs_elt_inv in Heq2 as [(l', l'') ->].
  ...
  2:{ cbn. clear. symmetry. etransitivity; [ apply PermutationT_middle | ].
      apply PermutationT_app_head, PermutationT_swap. }
  eapply IHA2.
  + assumption.
  + apply PermutationT_swap.
  + eapply ex_r; [ apply pi1' | ].
    list_simpl in HP2; cons2app in HP2; apply PermutationT_app_inv in HP2.
    list_simpl; etransitivity; [ symmetry; apply PermutationT_middle | ].
    symmetry; apply PermutationT_cons, HP2; reflexivity.
- eapply tens_rev in pi1; [ | intros [] | intro; reflexivity ].
  destruct pi1 as [_ pi1'].
  clear - pi1'.
  assert ({ l & PermutationT (covar (i2a atN) :: nil) l }) as [l HP] by (eexists; reflexivity).
  eapply ex_r in pi1'; [ | apply HP ].
  revert HP; induction pi1'; intros HP;
    try (now (apply PermutationT_length in HP; inversion HP));
    try (now (apply PermutationT_length_1_inv in HP; inversion HP)).
  + apply IHpi1'.
  ...
  + apply IHpi1'.
    ...
    apply PermutationT_app_head, PermutationT_app_tail, PermutationT_map, p.
  + destruct L; [ | discriminate f ].
    apply PermutationT_sym, PermutationT_nil_cons in HP as [].
- eapply tens_rev in pi1; [ | intros [] | intro; reflexivity ].
  cons2app in HP2.
  assert (Heq2 := HP2).
  symmetry in Heq2. apply PermutationT_vs_elt_inv in Heq2 as [(l', l'') ->].
  ...
  eapply (@cut_mix0_r _ _ (l' ++ ill2ll A2 :: l'')) in pi1'';
    [ | eapply ex_r; [ apply pi2 | symmetry; apply PermutationT_middle ]].
  eapply IHA2.
  + eassumption.
  + apply PermutationT_swap.
  + eapply ex_r; [ apply pi1'' | ].
...
- eapply tens_rev in pi1; [ | intros [] | intro; reflexivity ].
  ...
  assert ({ l & PermutationT (covar (i2a atN) :: nil) l })
    as [l HP] by (eexists; reflexivity).
  eapply ex_r in pi1'; [ | apply HP ].
  induction pi1' in HP |- *;
    try (apply PermutationT_length in HP; discriminate HP);
    try now apply PermutationT_length_1_inv in HP.
  + apply IHpi1'.
    symmetry in p. etransitivity; [ exact HP | exact p ].
  + apply IHpi1'.
    symmetry in p. etransitivity; [ exact HP | ].
...
- revert HP2. clear - pi2. induction pi2; intros HP2;
  ...
  + destruct L; [ | discriminate f ].
    apply PermutationT_sym, PermutationT_nil_cons in HP2 as [].
  + apply PermutationT_length_2_inv in HP2 as [ | ]; inversion e.
    destruct l; discriminate H1.
- assert (pi0 := pi1).
  ...
  { revert HP2. clear - pi2. induction pi2; intros HP2;
    ...
    - destruct L; [ | discriminate f ].
      apply PermutationT_sym, PermutationT_nil_cons in HP2 as [].
    - apply PermutationT_length_2_inv in HP2 as [ | ]; inversion e. subst.
      left. eapply ex_r; [ apply pi2 | ]. cons2app. apply PCPermutationT_app_comm.
    - apply PermutationT_length_2_inv in HP2 as [ | ]; inversion e. subst.
      right. eapply ex_r; [ apply pi2 | ]. cons2app. apply PCPermutationT_app_comm.
    - apply PermutationT_length_2_inv in HP2 as [ | ]; inversion e.
      destruct l; discriminate H1. }
...
- revert HP2. clear - pi2. induction pi2; intros HP2;
  ...
  + destruct L; [ | discriminate f ].
    apply PermutationT_sym, PermutationT_nil_cons in HP2 as [].
  + apply PermutationT_length_2_inv in HP2 as [ | ]; inversion e; destruct l; discriminate H1.
Qed.
...
- list_simpl in pi2. eapply ex_r in pi2; [ | apply PermutationT_swap ].
...
- apply PCPermutationT_map_inv in p as [ l1' -> HP ].
  symmetry in HP.
  apply PCPermutationT_map_inv in HP as [ l1'' -> HP ].
  eapply IHpi; [ reflexivity | ].
  eapply PermutationT_ForallT; [ apply HP | eassumption ].
- decomp_map_eq Heq eqn:Heq'. decomp_map_eq Heq. subst.
  symmetry in Heq'. destruct (ill2ll_map_ioc_inv _ _ Heq') as [l' -> ->].
  rewrite map_map in p. apply PermutationT_map_inv in p as [l'' -> p].
  rewrite <- (map_map _ _ l''), <- ill2ll_map_ioc, <- ? map_app in IHpi.
  eapply IHpi; [ reflexivity | ].
  assert (HF1 := ForallT_app_l _ _ HF).
  assert (HF2 := ForallT_app_r _ _ HF).
  assert (HF3 := ForallT_app_l _ _ HF2).
  assert (HF4 := ForallT_app_r _ _ HF2).
  apply ForallT_app; [ assumption | ].
  apply ForallT_app; [ | assumption ].
  apply (PermutationT_map ioc) in p.
...
- destruct l'; inversion Heq.
  symmetry in H1. decomp_map_eq H1. symmetry in H1. decomp_map_eq H1. subst.
  destruct i; inversion H0.
  + inversion HF. subst.
    cbn in X. rewrite <- (app_nil_l _) in X. eapply parr_rev in X; [ | intros [] ].
    list_simpl in X.
    assert (X1 := ForallT_app_l _ _ X0).
    assert (X2 := ForallT_app_r _ _ X0).
    rewrite bidual in pi1.
    ...
    { eapply (stronger_pfrag _ pfrag_mix0) in pi1; [ | repeat split; intros [] ].
      ...
      - assumption.
      - inversion HF. subst.
        cbn in pi. eapply ex_r in pi; [ | apply PermutationT_swap ].
        apply (cut_mix0_r pi) in X.
        apply IHl1; assumption. }
    eapply ex_r in X; [ | apply PermutationT_swap ].
...
- destruct l'; inversion Heq.
  ...
  inversion HF. subst.
  clear - X.
  assert ({ l & PermutationT (ill2ll izero :: nil) l }) as [l HP]
...
- subst.
  ...
  apply PCPermutationT_map_inv in p as [l' -> HP].
  apply PCPermutation_PermutationT in HP; unfold id in HP.
  apply (PermutationT_ForallT HP) in Hnzsp.
  apply IHHll in Hnzsp as [[[C l1] l2] Hz ->]; [ | rewrite <- map_map; reflexivity ].
  apply PermutationT_vs_elt_inv in HP as [(l', l'') ->].
  exists (C, l', l''); [ assumption | reflexivity ].
- decomp_map_eq HP eqn:Heq. decomp_map_eq HP. subst.
  symmetry in Heq. apply ill2ll_map_ioc_inv in Heq as [lw'' -> ->].
  rewrite map_map in p.
  apply PermutationT_map_inv in p as [lw''' -> p].
  assert (ForallT nonzerospos (l1 ++ map ioc lw''' ++ l2)) as HF.
  { assert (HF1 := ForallT_app_l _ _ Hnzsp).
    assert (HF2 := ForallT_app_r _ _ Hnzsp).
    assert (HF3 := ForallT_app_l _ _ HF2).
    assert (HF4 := ForallT_app_r _ _ HF2).
    apply ForallT_app; [ assumption | ].
    apply ForallT_app; [ | assumption ].
    apply (PermutationT_map ioc) in p.
    eapply PermutationT_ForallT; [ exact p | assumption ]. }
  ...
  + decomp_elt_eq_app Heq; subst; cbn.
    * apply (PermutationT_map ioc) in p. rewrite <- Heq in p.
      apply PermutationT_vs_elt_inv in p as [(llw1, llw2) ->].
...
- destruct l0; inversion HP.
  destruct i; discriminate H0.
- rewrite map_map in HP. decomp_map_eq HP. subst.
...
- rewrite map_map in HP. decomp_map_eq HP eqn:Heq. subst.
  destruct x; inversion Heq; subst.
  + rewrite <- map_map in IHHll2.
    assert (ForallT nonzerospos (x2 :: l2)) as Hnzsp'.
    { inversion_clear Hnzsp as [ | ? ? Hnzsp' HF].
      apply ForallT_app_l in HF.
      inversion Hnzsp'.
    ...
    * exfalso. inversion_clear Hnzsp as [ | ? ? Hnzsp' ]. inversion Hnzsp'.
      apply H0; assumption.
    * exists (C, ilpam x1 i :: l1', l2' ++ l1); [ assumption | list_reflexivity ].
  + assert (ForallT nonzerospos (N :: l2)) as Hnzsp'.
    ...
    * exists (C, igen x :: l1', l2' ++ l1); [ assumption | list_reflexivity ].
  + assert (ForallT nonzerospos (x2 :: l1)) as Hnzsp'.
    { inversion_clear Hnzsp as [ | ? ? Hnzsp' HF ].
      apply ForallT_app_r in HF.
      inversion Hnzsp'.
      constructor; assumption. }
    ...
    * exfalso. inversion_clear Hnzsp as [ | ? ? Hnzsp' ]. inversion Hnzsp'.
      apply H0; assumption.
    * exists (C, ilmap x1 i :: l2 ++ l1', l2'); [ assumption | list_reflexivity ].
  + assert (ForallT nonzerospos (N :: l1)) as Hnzsp'.
    { inversion_clear Hnzsp as [ | ? ? Hnzsp' HF ].
      apply ForallT_app_r in HF.
      inversion Hnzsp'.
      constructor; [ constructor | assumption ]. }
    ...
    destruct l1'; inversion Heq2; subst.
    * exfalso. inversion Hzp.
    * exists (C, ineg x :: l2 ++ l1', l2'); [ assumption | list_reflexivity ].
- rewrite map_map in HP. decomp_map_eq HP eqn:Heq. subst.
  destruct x; inversion Heq. subst.
  rewrite <- map_map in IHHll.
  assert (ForallT nonzerospos (x2 :: x1 :: l)) as Hnzsp'.
...
- decomp_map_eq HP eqn:Ht. decomp_map_eq HP. subst.
  destruct x; inversion Ht.
  exists (izero, nil, l); constructor.
- rewrite map_map in HP. decomp_map_eq HP eqn:Hp. subst.
  destruct x; inversion Hp. subst.
  rewrite <- map_map in IHHll.
  assert (ForallT nonzerospos (x1 :: l)) as Hnzsp'.
  { inversion_clear Hnzsp as [ | ? ? Hnzsp' ]. inversion_clear Hnzsp'.
    constructor; assumption. }
  apply IHHll in Hnzsp' as [[[C l1'] l2'] Hzp Heq2]; [ | reflexivity ].
  destruct l1'; inversion Heq2; subst.
...
- rewrite map_map in HP. decomp_map_eq HP eqn:Hp. subst.
  destruct x; inversion Hp. subst.
  rewrite <- map_map in IHHll.
  assert (ForallT nonzerospos (x2 :: l)) as Hnzsp'.
  { inversion_clear Hnzsp as [ | ? ? Hnzsp' ]. inversion_clear Hnzsp'.
    constructor; assumption. }
  apply IHHll in Hnzsp' as [[[C l1'] l2'] Hzp Heq2]; [ | reflexivity ].
  destruct l1'; inversion Heq2; subst.
...
- rewrite map_map in HP. decomp_map_eq HP eqn:Hw. subst.
  ...
  apply IHHll2 in Hnzsp' as [[[C l1'] l2'] Hzp Heq2]; [ | reflexivity ].
  destruct l1'; inversion Heq2; subst.
  + assert (ForallT nonzerospos (x1 :: l2')) as Hnzsp''.
    { inversion_clear Hnzsp as [ | ? ? Hnzsp' ]. inversion_clear Hnzsp'.
      constructor; assumption. }
    rewrite <- map_map in IHHll1.
...
- exfalso.
  destruct l0; inversion HP.
  destruct i; inversion H0.
- rewrite map_map in HP. decomp_map_eq HP eqn:Hwn. subst.
  destruct x; inversion Hwn. subst.
  rewrite <- map_map in IHHll.
  assert (ForallT nonzerospos (x :: l)) as Hnzsp'.
  { inversion_clear Hnzsp as [ | ? ? Hnzsp' ]. inversion_clear Hnzsp'.
    constructor; assumption. }
  apply IHHll in Hnzsp' as [[[C l1'] l2'] Hzp Heq2]; [ | reflexivity ].
  destruct l1'; inversion Heq2; subst.
...
- rewrite map_map in HP. decomp_map_eq HP eqn:Hwn. subst.
  destruct x; inversion Hwn. subst.
  assert (ForallT nonzerospos (ioc x :: ioc x :: l)) as Hnzsp'.
...
- unfold i2pfrag in HP. cbn in HP.
  ...
  + list_simpl in H1. rewrite H2 in H1. list_simpl in H1.
    ...
    decomp_map_eq H1 eqn:Hd. destruct Hd as [Hd1 [Hd2 Hd3]]. apply dual_inj in Hd2 as ->.
    decomp_map_eq H1 eqn:Hxz. subst. symmetry in Hxz. apply (ill2ll_zeropos _ Hz) in Hxz.
...
Lemma ll_to_ill_nzeropos_cutfree P (Hcut : no_icut P) (Hgax : easyipgax_nzeropos P) l :
  ll (i2pfrag P) l -> forall l0 C, ForallT nonzerospos (C :: l0) ->
  PCPermutationT (pperm (i2pfrag P)) l (ill2ll C :: rev (map dual (map ill2ll l0))) ->
  ill P l0 C.
Proof.
intros Hll. induction Hll; intros l0 C Hnzsp HP.
- apply PCPermutationT_length_2_inv in HP as [HP | HP]; inversion HP; destruct C; inversion H0.
...
- apply PCPermutationT_vs_cons_inv in HP as [[l1' l2'] HP Heq].
  decomp_elt_eq_app Heq; subst.
  + apply PEPermutationT_rev in HP. rewrite rev_involutive in HP.
    rewrite ? rev_app_distr, <- map_rev, <- ? app_assoc, map_map in HP.
    symmetry in HP. apply PEPermutationT_map_inv in HP as [l' Heq HP].
    remember (rev l1') as l1r.
    remember (rev l2) as l2r.
    remember (map wn (rev lw')) as lw'r.
    remember (rev l) as lr.
    decomp_map_eq Heq eqn:Heq'. subst. destruct Heq' as [Heq0 [Heq1 [Heq2 Heq3]]].
    ...
    rewrite app_assoc. apply PermutationT_rev' in p. rewrite Heq, map_map in p.
    apply PermutationT_map_inv in p as [lw'' Heq' p].
    symmetry in p. eapply ex_oc_ir; [ | eassumption ].
    apply IHHll.
    * inversion_clear Hnzsp as [ | ? ? ? HF ].
      cbn in HP. symmetry in HP.
      apply (PEPermutationT_ForallT _ _ HP) in HF.
      assert (HF1:= ForallT_app_l _ _ HF).
      assert (HF2:= ForallT_app_r _ _ HF).
      assert (HF3:= ForallT_app_l _ _ HF2).
      assert (HF4:= ForallT_app_r _ _ HF2).
      assert (HF5:= ForallT_app_l _ _ HF4).
      assert (HF6:= ForallT_app_r _ _ HF4).
      constructor; [ assumption | ].
      apply ForallT_app; apply ForallT_app; try assumption.
      symmetry in p. apply (PermutationT_map ioc) in p.
      eapply PermutationT_ForallT; eassumption.
  ...
  + decomp_elt_eq_app Heq; subst;
      [ exfalso; decomp_map_eq Heq eqn:Heq'; destruct C; inversion Heq' | ].
    apply PEPermutationT_rev in HP. rewrite rev_involutive in HP. list_simpl in HP. rewrite map_map in HP.
    symmetry in HP. apply PEPermutationT_map_inv in HP as [l' Heq HP].
    remember (rev l3) as l3r.
    remember (map wn (rev lw')) as lw'r.
    remember (rev l1) as l1r.
    remember (rev l2') as l2'r.
    decomp_map_eq Heq eqn:Heq'. subst. destruct Heq' as [Heq1 [Heq2 [Heq3 Heq4]]].
    ...
    apply PermutationT_rev' in p. rewrite Heq, map_map in p.
    apply PermutationT_map_inv in p as [lw'' Heq' p].
    symmetry in p. eapply ex_oc_ir; [ | eassumption ].
    apply IHHll.
    * inversion_clear Hnzsp as [ | ? ? ? HF ].
      ...
      assert (HF6:= ForallT_app_r _ _ HF4).
      constructor; [ assumption | ].
      rewrite app_assoc. apply ForallT_app; apply ForallT_app; try assumption.
      symmetry in p. apply (PermutationT_map ioc) in p.
      eapply PermutationT_ForallT; eassumption.
    * list_simpl. cbn in HP.
      ...
      symmetry. etransitivity; [ apply PCPermutationT_app_comm | list_reflexivity ].
- discriminate f.
- apply PCPermutationT_length_1_inv in HP as [= HP1 HP2].
  destruct C; inversion HP1.
  destruct l0 using rev_rect; [ | list_simpl in HP2; discriminate HP2 ].
  apply one_irr.
- list_simpl in HP. symmetry in HP.
  apply PCPermutationT_vs_cons_inv in HP as [(l', l'') HP Heq].
  destruct l'; inversion Heq.
  + destruct C; discriminate H0.
  + decomp_map_eq H1 eqn:Hx. decomp_map_eq H1. subst.
    ...
    * inversion Hnzsp as [ | ? ? ? HF ].
      constructor; [ assumption | ].
      list_simpl in HF.
      assert (H3l := ForallT_app_l _ _ HF).
      assert (H3r := ForallT_app_r _ _ HF).
      inversion H3r.
      apply ForallT_app; assumption.
    * list_simpl.
      apply PEPermutation_PCPermutationT in HP. unfold id in HP. cbn in HP.
      etransitivity; [ apply HP | rewrite app_comm_cons; apply PCPermutationT_app_comm ].
- list_simpl in HP. symmetry in HP.
  apply PCPermutationT_vs_cons_inv in HP as [(l', l'') HP Heq].
  destruct l'; inversion Heq.
  + destruct C; inversion H0. subst.
    list_simpl in HP. rewrite map_map in HP.
    apply PEPermutationT_map_inv in HP as [l3 Heq' HP].
    rewrite <- map_map in Heq'. decomp_map_eq Heq'. decomp_map_eq Heq'. subst.
    inversion_clear Hnzsp as [ | ? ? Hnzsp' HF ]. inversion_clear Hnzsp'.
    apply ForallT_rev, (PEPermutationT_ForallT _ _ HP) in HF.
    assert (H3l := ForallT_app_l _ _ HF).
    assert (H3r := ForallT_app_r _ _ HF).
    apply PEPermutationT_rev in HP. list_simpl in HP. symmetry in HP.
    eapply ex_ir; [ | exact HP ].
    apply tens_irr.
    * apply IHHll1.
      -- constructor; [ | apply ForallT_rev ]; assumption.
      -- list_simpl. reflexivity.
    * apply IHHll2.
      -- constructor; [ | apply ForallT_rev ]; assumption.
      -- list_simpl. reflexivity.
  + decomp_map_eq H1 eqn:Ht. decomp_map_eq H1. subst.
    inversion_clear Hnzsp as [ | ? ? ? HF ].
    apply ForallT_rev in HF. rewrite H1 in HF.
    assert (H3l := ForallT_app_l _ _ HF).
    assert (H3r := ForallT_app_r _ _ HF).
    inversion H3r as [ | ? ? Hnzsp' HF']. subst.
    apply (ForallT_app HF') in H3l.
    assert ({'(ll, lr) & 
       PEPermutationT (ipperm P) (l'' ++ l') (ll ++ lr) &
       l2 ++ l1 = map dual (map ill2ll ll)
                         ++ ill2ll C :: map dual (map ill2ll lr) /\
       (ipperm P = false -> l'' = ll /\ l' = lr) }) as [(ll, lr) HP0 [Heq' HPeq]].
    { clear - HP.
      destruct (ipperm P).
      - apply PEPermutationT_vs_elt_inv in HP as [(ll & lr) HP0 Heq].
        cbn in HP0. rewrite <- 2 map_app, map_map in HP0. apply PermutationT_map_inv in HP0 as [l3 Heq1 HP].
        rewrite <- map_map in Heq1. decomp_map_eq Heq1. decomp_map_eq Heq1. subst.
        exists (ll, lr); [ | split ]; [ assumption | assumption | ].
        intros [=].
      - cbn in HP. exists (l'', l'); cbn; [ | repeat split ]; [ reflexivity | assumption ]. }
    decomp_elt_eq_app Heq'; subst.
    * decomp_map_eq Heq'0. decomp_map_eq Heq'0. subst.
      apply (PEPermutationT_ForallT _ _ HP0) in H3l. cbn in H3l.
      ...
      -- apply (f_equal (@rev _)) in H1. rewrite rev_involutive in H1. subst.
         ...
         ++ apply lpam_ilr.
            ** apply IHHll1.
               --- constructor; [ assumption | ].
                   apply ForallT_app_r, ForallT_app_r in H3l.
                   apply ForallT_rev. assumption.
               --- rewrite bidual. list_simpl. reflexivity.
            ** apply IHHll2.
               --- constructor; [ assumption | ].
                   assert (H3l' := ForallT_app_l _ _ H3l).
                   assert (H3l'' := ForallT_app_r _ _ H3l).
                   apply ForallT_app_l, ForallT_rev in H3l''.
                   apply ForallT_rev in H3l'.
                   apply ForallT_app; [ assumption | ].
                   constructor; assumption.
               --- list_simpl. rewrite ? app_comm_cons. apply PCPermutationT_app_comm.
         ++ destruct (ipperm P).
            ** clear - HP0.
               apply PermutationT_rev' in HP0.
               list_simpl in HP0. list_simpl.
               apply PermutationT_elt.
               symmetry. etransitivity; [ apply PermutationT_app_comm | ].
               etransitivity; [ exact HP0 | ].
               etransitivity; [ | apply PermutationT_app_comm ]. rewrite app_assoc. reflexivity.
         ...
         ++ apply gen_pam_rule.
            ...
            ** apply IHHll2.
               --- constructor; [ assumption | ].
                   assert (H3l' := ForallT_app_l _ _ H3l).
                   assert (H3l'' := ForallT_app_r _ _ H3l).
                   apply ForallT_app_l in H3l''.
                   apply ForallT_rev in H3l'.
                   apply ForallT_rev in H3l''.
                   apply ForallT_app; [ | constructor; [ constructor | ] ]; assumption.
               --- list_simpl. rewrite ? app_comm_cons. apply PCPermutationT_app_comm.
         ++ destruct (ipperm P).
            ** clear - HP0.
               apply PermutationT_rev' in HP0.
               ...
               apply PermutationT_elt.
               symmetry. etransitivity; [ apply PermutationT_app_comm | ].
               etransitivity; [ exact HP0 | rewrite app_assoc; apply PermutationT_app_comm ].
      ...
      -- change (dual (ill2ll x2) :: map dual (map ill2ll l1))
         ...
         ++ destruct Hll1 as [[[C1 [|]] lz2] HzC1 Heq1]; inversion Heq1; subst.
            ...
            ** apply (f_equal (@rev _)) in H1. rewrite rev_involutive in H1. subst.
               ...
               apply PEPermutationT_vs_elt_inv in HP0 as [(ll1, lr1) _ HP0].
         ...
         ++ constructor; [ assumption | ].
            apply ForallT_app_r in H3l.
            apply ForallT_app_r in H3l. assumption.
      -- change (covar (i2a atN) :: map dual (map ill2ll l1))
         ...
         ++ destruct Hll1 as [[[C1 [|]] lz2] HzC1 Heq1]; inversion Heq1; subst.
            ...
            ** apply (f_equal (@rev _)) in H1. rewrite rev_involutive in H1. subst.
               ...
               apply PEPermutationT_vs_elt_inv in HP0 as [(ll1, lr1) _ Heq0].
         ...
         ++ constructor.
            ...
            --- apply ForallT_app_r in H3l.
                apply ForallT_app_r in H3l; assumption.
    * decomp_map_eq Heq'0. decomp_map_eq Heq'0. subst.
      apply (PEPermutationT_ForallT _ _ HP0) in H3l.
      destruct x; inversion Hnzsp'; inversion Ht; subst.
      -- change (dual (ill2ll x2) :: map dual (map ill2ll l2))
         ...
         ++ destruct Hll2 as [[[C1 [|]] lz2] HzC1 Heq1]; inversion Heq1; subst.
            ** contradiction HzC1.
            ** apply (f_equal (@rev _)) in H1. rewrite rev_involutive in H1. subst.
               list_simpl in HP0. apply PEPermutationT_vs_elt_inv in HP0 as [(ll1, lr1) _ Heq0].
               decomp_elt_eq_app Heq0; subst; list_simpl.
               --- apply zeropos_ilr. assumption.
               --- rewrite ? app_comm_cons, ? app_assoc. apply zeropos_ilr. assumption.
         ++ constructor; [ assumption | ].
            apply ForallT_app_l, ForallT_app_l in H3l. assumption.
      ...
      -- apply (f_equal (@rev _)) in H1. rewrite rev_involutive in H1. subst.
         ...
         ++ apply lmap_ilr.
            ** apply IHHll2.
               --- constructor; [ assumption | ].
                   ...
                   apply ForallT_app_l in H3l.
                   apply ForallT_rev; assumption.
               --- rewrite bidual; list_reflexivity.
            ** apply IHHll1.
               --- constructor; [ assumption | ].
                   assert (H3l' := ForallT_app_l _ _ H3l).
                   assert (H3l'' := ForallT_app_r _ _ H3l).
         ...
         ++ destruct (ipperm P) eqn:Hperm.
            ** clear - HP0.
               ...
               symmetry. etransitivity; [ apply PermutationT_app_comm | exact HP0 ].
            ** destruct (HPeq eq_refl). subst. list_simpl. reflexivity.
      -- apply (f_equal (@rev _)) in H1. rewrite rev_involutive in H1. subst.
         ...
         ++ apply neg_map_rule.
            ...
            ** apply IHHll1.
               ...
               --- list_simpl. rewrite ? app_comm_cons. apply PCPermutationT_app_comm.
         ++ destruct (ipperm P) eqn:Hperm.
            ** clear - HP0. apply PermutationT_rev' in HP0.
               list_simpl in HP0. list_simpl. rewrite app_assoc.
               apply PermutationT_elt.
               etransitivity; [ apply PermutationT_app_comm | ].
               symmetry. etransitivity; [ apply PermutationT_app_comm | exact HP0 ].
            ** destruct (HPeq eq_refl). subst. list_simpl. reflexivity.
- list_simpl in HP. symmetry in HP.
  apply PCPermutationT_vs_cons_inv in HP as [([|] & l'') HP Heq].
  + inversion Heq as [ Heq0 ]; subst.
    list_simpl in HP.
    rewrite map_map in HP.
    apply PEPermutationT_map_inv in HP.
    destruct HP as [l3 Heq' HP].
    rewrite <- map_map in Heq'. subst.
    inversion_clear Hnzsp as [ | ? ? Hnzsp' HF ].
    apply ForallT_rev in HF.
    apply (PEPermutationT_ForallT _ _ HP) in HF.
    destruct C; inversion Hnzsp'; inversion Heq0; subst.
    * apply lpam_irr.
      symmetry in HP.
      apply PEPermutationT_rev in HP.
      rewrite rev_involutive in HP.
      apply (ex_ir (rev l3 ++ C1 :: nil));
        [ | apply PEPermutationT_add_inside; [ assumption | reflexivity ] ].
      apply IHHll.
      -- constructor; [ assumption | ].
         apply ForallT_app; [ | constructor ]; try constructor; [ | assumption ].
         apply ForallT_rev; assumption.
      -- list_reflexivity.
    * apply gen_irr.
      symmetry in HP.
      ...
      apply (ex_ir (rev l3 ++ C :: nil));
        [ | apply PEPermutationT_app; [ assumption | reflexivity ] ].
      apply IHHll.
      -- constructor; [ constructor | ].
         apply ForallT_app; [ | constructor; [ | constructor ] ]; [ | assumption ].
         apply ForallT_rev. assumption.
    ...
    * apply lmap_irr.
      ...
      apply PEPermutationT_rev in HP.
      rewrite rev_involutive in HP.
      apply (ex_ir (C1 :: rev l3)); [ | apply PEPermutationT_cons, HP; reflexivity ].
      apply IHHll.
      -- constructor; [ assumption | constructor; [ assumption | ] ].
         apply ForallT_rev. assumption.
      -- list_simpl. symmetry. rewrite app_comm_cons.
         etransitivity; [ apply PCPermutationT_app_comm | reflexivity ].
    * apply neg_irr.
      symmetry in HP.
      apply PEPermutationT_rev in HP.
      rewrite rev_involutive in HP.
      apply (ex_ir (C :: rev l3)); [ | apply PEPermutationT_cons, HP; reflexivity ].
      apply IHHll.
      -- constructor; [ constructor | ].
         constructor; [ assumption | ].
         apply ForallT_rev. assumption.
      -- list_simpl. rewrite app_comm_cons.
         symmetry. etransitivity; [ apply PCPermutationT_app_comm | reflexivity ].
  + inversion Heq as [[Heq0 Heq1]]. subst.
    decomp_map_eq Heq1 eqn:Heq0. decomp_map_eq Heq1. subst.
    apply (f_equal (@rev _)) in Heq1. rewrite rev_involutive in Heq1. subst.
    ...
    * inversion Hnzsp as [ | ? ? ? HF ].
      ...
      constructor; [ | constructor ]; assumption.
    * list_simpl.
      etransitivity; [ | rewrite ? app_comm_cons; apply PCPermutationT_app_comm].
      apply PEPermutation_PCPermutationT.
      apply PEPermutationT_cons; [ reflexivity | ].
      apply PEPermutationT_cons; [ reflexivity | exact HP ].
- list_simpl in HP. symmetry in HP.
  apply PCPermutationT_vs_cons_inv in HP as [([|] & l'') HP Heq]; inversion Heq; subst.
...
- list_simpl in HP. symmetry in HP.
  apply PCPermutationT_vs_cons_inv in HP as [([|] & l'') HP Heq]; inversion Heq; subst.
  + list_simpl in HP. rewrite map_map in HP. apply PEPermutationT_map_inv in HP as [l3 Heq' HP].
    rewrite <- map_map in Heq'. subst.
    inversion_clear Hnzsp as [ | ? ? Hnzsp' HF].
    apply ForallT_rev in HF.
    apply (PEPermutationT_ForallT _ _ HP) in HF.
    ...
    symmetry in HP. apply PEPermutationT_rev in HP. rewrite rev_involutive in HP.
    apply (ex_ir (rev l3)); [ | apply HP ].
    apply plus_irr1, IHHll.
    * constructor; [ assumption | ].
      apply ForallT_rev; assumption.
    * list_simpl. reflexivity.
  + decomp_map_eq H1 eqn:Hx. decomp_map_eq H1.
    ...
    * inversion Hnzsp as [ | ? ? ? HF ].
      ...
      apply ForallT_app; [ assumption | ].
      constructor; assumption.
    * list_simpl. rewrite HP. rewrite ? app_comm_cons. apply PCPermutationT_app_comm.
- list_simpl in HP. symmetry in HP.
  apply PCPermutationT_vs_cons_inv in HP as [([|] & l'') HP Heq]; inversion Heq; subst.
  + list_simpl in HP. rewrite map_map in HP. apply PEPermutationT_map_inv in HP as [l3 Heq' HP].
    rewrite <- map_map in Heq'. subst.
...
- list_simpl in HP. symmetry in HP.
  ...
  + list_simpl in HP. rewrite map_map in HP. apply PEPermutationT_map_inv in HP as [l3 Heq' HP].
    ...
    * constructor; [ assumption | ].
      apply ForallT_rev in HF; list_simpl in HF; assumption.
...
- list_simpl in HP. symmetry in HP.
  ...
  + decomp_map_eq H1 eqn:Hx. decomp_map_eq H1. subst.
    ...
    * inversion Hnzsp as [ | ? ? ? HF ].
      ...
      inversion_clear Hnzsp'.
      apply ForallT_app; [ | constructor ]; assumption.
    * list_simpl. rewrite HP, ? app_comm_cons. apply PCPermutationT_app_comm.
- list_simpl in HP. symmetry in HP.
  apply PCPermutationT_vs_cons_inv in HP as [([|], l'') HP Heq]; inversion Heq; subst.
  + destruct C; discriminate H0.
  + decomp_map_eq H1 eqn:Hx. decomp_map_eq H1. subst.
    ...
    * inversion Hnzsp as [ | ? ? ? HF ].
      ...
      apply ForallT_app; assumption.
    * list_simpl.
      apply PEPermutation_PCPermutationT in HP. unfold id in HP.
      etransitivity; [ exact HP | rewrite app_comm_cons; apply PCPermutationT_app_comm ].
- list_simpl in HP. symmetry in HP.
  apply PCPermutationT_vs_cons_inv in HP as [([|], l'') HP Heq]; inversion Heq; subst.
  + destruct C; discriminate H0.
  + decomp_map_eq H1 eqn:Hx. decomp_map_eq H1. subst.
    ...
    * inversion_clear Hnzsp as [ | ? ? ? HF ].
      ...
      constructor; [ assumption | ].
      apply ForallT_app; [ | constructor ]; assumption.
    * apply (PEPermutationT_cons _ (eq_refl (wn (dual (ill2ll x))))) in HP.
      apply (PEPermutationT_cons _ (eq_refl (wn (dual (ill2ll x))))) in HP.
      apply PEPermutation_PCPermutationT in HP. unfold id in HP.
...
Lemma ll_to_ill_nzeropos_axfree P (P_axfree : no_iax P) l C :
  ForallT nonzerospos (C :: l) -> ll (i2pfrag P) (ill2ll C :: rev (map dual (map ill2ll l))) -> ill P l C.
Proof.
intros Hnz pi.
...
Definition easyipgax_oclmap P := forall a,
  (forall A, InT (dual (ill2ll A)) (map ill2ll (fst (projT2 (ipgax P) a))) -> notT (oclike A))
* (forall A, ill2ll A = ill2ll (snd (projT2 (ipgax P) a)) -> notT (oclike A))
* (forall l C,
     PCPermutationT (ipperm P) (ill2ll (snd (projT2 (ipgax P) a))
                         :: rev (map dual (map ill2ll (fst (projT2 (ipgax P) a)))))
                       (ill2ll C :: rev (map dual (map ill2ll l)))
       -> ill P l C)
* notT (InT N (fst (projT2 (ipgax P) a))).
...
Lemma ll_to_ill_oclpam_cutfree P (P_perm : ipperm P = true) (P_cut : no_icut P) (Hgax : easyipgax_oclmap P) l :
  ll (i2pfrag P) l -> forall l0 l1 C, ForallT oclpam (C :: l0) ->
    ForallT oclike l1 ->
    PCPermutationT (pperm (i2pfrag P)) l
...
- apply PCPermutationT_length_2_inv in HP.
...
- assert (PCPermutationT (pperm (i2pfrag P)) (l1 ++ map wn lw ++ l2)
...
- apply PCPermutationT_length_1_inv in HP.
  ...
  + apply one_irr.
  + intros Hnil. contradiction Hnil. reflexivity.
- symmetry in HP. apply PCPermutationT_vs_cons_inv in HP as [(l', l'') HP Heq].
  destruct l'; inversion Heq; [ destruct C; inversion H0 | ]. subst.
  symmetry in H1. decomp_elt_eq_app H1; subst.
  { decomp_map_eq H1 eqn:Hx. destruct x; destr_eq Hx. }
  list_simpl in H1. decomp_map_eq H1 eqn:Hx. decomp_map_eq H1.
  apply (f_equal (@rev _)) in H1. list_simpl in H1. subst.
  destruct x; destr_eq Hx.
  apply PEPermutation_PCPermutationT in HP. unfold id in HP.
  assert (HP' := PCPermutationT_trans _ _ _ _ HP (PCPermutationT_app_comm _ _ _)).
  ...
  + inversion Hoclm as [ | ? ? ? HF ]; constructor; [ assumption | ].
    assert (Hl := ForallT_app_l _ _ HF).
    assert (Hr := ForallT_app_r _ _ HF). inversion_clear Hr.
    apply ForallT_app; assumption.
- symmetry in HP. apply PCPermutationT_vs_cons_inv in HP as [(l', l'') HP Heq].
  cbn in HP. rewrite P_perm in HP. cbn in HP.
  apply PermutationT_app_app_inv in HP as [[[[l3' l3''] l4'] l4''] [[HP1 HP2] [HP3 HP4]]].
  destruct l'; inversion Heq; [ destruct C; inversion H0 | ]; subst.
  + apply PermutationT_nil in HP4. apply app_eq_nil in HP4 as [-> ->].
    list_simpl in HP1. symmetry in HP1.
    list_simpl in HP2. symmetry in HP2.
    apply (@PermutationT_trans _ _ _ (l2 ++ l1)) in HP3;
      [ | apply PermutationT_app ]; try assumption.
    clear l3' l3'' HP1 HP2.
    apply PermutationT_app_app_inv in HP3 as [[[[l3' l3''] l4'] l4''] [[HP1 HP2] [HP3 HP4]]].
    symmetry in HP1. apply PermutationT_map_inv in HP1 as [l3 Heq1 HP1].
    decomp_map_eq Heq1. subst.
    list_simpl in HP2. rewrite map_map in HP2.
    symmetry in HP2. apply PermutationT_map_inv in HP2 as [l3 Heq2 HP2].
    apply PermutationT_rev' in HP2. rewrite rev_involutive in HP2.
    rewrite <- map_map in Heq2. decomp_map_eq Heq2. decomp_map_eq Heq2. subst.
    specialize IHHll1 with (rev l4'') l4' C1.
    specialize IHHll2 with (rev l3'') l3' C2.
    apply (PermutationT_ForallT HP1) in Hocl.
    assert (Hocl2 := ForallT_app_l _ _ Hocl).
    assert (Hocl1 := ForallT_app_r _ _ Hocl).
    ...
    * destruct Hocl1 as [IH1 IH2].
      ...
      -- intros Hnil lw.
         ...
         symmetry in HP1. apply PermutationT_vs_cons_inv in HP1 as [(ll, lr) Heq3].
         ...
         ++ assert (ll ++ i :: l <> nil) as Hnil2
            ...
            eapply ex_ir; [apply tens_irr; eassumption | ].
            list_simpl in HP2.
            rewrite P_perm. cbn. rewrite app_assoc. apply PermutationT_app_tail. symmetry. assumption.
         ++ assert (l ++ i :: lr <> nil) as Hnil2
              by (clear; intros H; destruct l; inversion H).
            assert (IH := IH2 Hnil2 lw).
            eapply ex_ir; [apply tens_irr; eassumption | ].
            list_simpl in HP2.
            rewrite P_perm. cbn. etransitivity; [ apply PermutationT_app_comm | ].
            rewrite app_assoc. apply PermutationT_app_tail.
            symmetry. etransitivity; [ exact HP2 | apply PermutationT_app_comm ].
    * inversion_clear Hoclm as [ | ? ? Hoclm' HF ].
      inversion_clear Hoclm'.
      constructor; try assumption.
      list_simpl in HP2.
      apply (PermutationT_ForallT HP2) in HF.
      apply ForallT_app_r in HF; assumption.
    * now cbn; rewrite P_perm; list_simpl; apply PermutationT_cons.
    * inversion Hoclm as [ | ? ? Hoclm' HF ].
      inversion Hoclm'.
      constructor; try assumption.
      list_simpl in HP2.
      apply (PermutationT_ForallT HP2) in HF.
      apply ForallT_app_l in HF; assumption.
    * now cbn; rewrite P_perm; list_simpl; apply PermutationT_cons.
  + decomp_elt_eq_app H1; subst.
    * decomp_map_eq H1 eqn:Hx. subst.
      destruct x; destr_eq Hx. subst.
      assert (HP4' := HP4).
      symmetry in HP4'. apply PermutationT_vs_cons_inv in HP4' as [(ll, lr) Heq'].
      apply PermutationT_app_app_inv in HP3 as [[[[l3l l3r] l4l] l4r] [[HP3' HP3''] [HP3''' HP3'''']]].
      ...
      apply PermutationT_map_inv in HP3'' as [l3 Heq'' HP3''].
      apply PermutationT_rev' in HP3''. rewrite rev_involutive in HP3''.
      rewrite <- map_map in Heq''.
      decomp_map_eq Heq''. decomp_map_eq Heq''. subst.
      apply (PermutationT_app_tail l4') in HP3'''.
      assert (HP1' := PermutationT_trans HP1 HP3''').
      apply (PermutationT_app_tail l4'') in HP3''''.
      assert (HP2' := PermutationT_trans HP2 HP3'''').
      decomp_elt_eq_app Heq'; subst.
      -- list_simpl in HP4. apply PermutationT_cons_app_inv in HP4.
         symmetry in HP4. apply PermutationT_map_inv in HP4 as [l5 Heq' HP].
         decomp_map_eq Heq'. subst.
         specialize IHHll2 with (rev l3r) (x2 :: l3l ++ ll ++ l3) C.
         assert (ForallT oclike (x2 :: l3l ++ ll ++ l3)) as Hocl'.
         { assert (Hocl2 := ForallT_app_l _ _ Hocl).
           assert (Hocl1 := ForallT_app_r _ _ Hocl).
           apply (PermutationT_ForallT HP) in Hocl2.
           inversion Hocl1 as [ | ? ? Hocl1' HF ].
           inversion_clear Hocl1'.
           constructor; [ assumption | ].
           cbn in HP3'. apply (PermutationT_ForallT HP3') in HF.
           assert (HF1 := ForallT_app_l _ _ HF).
           assert (HF2 := ForallT_app_r _ _ HF).
           apply ForallT_app; [ assumption | ].
           rewrite app_assoc in Hocl2.
           assert (Hocl3 := ForallT_app_l _ _ Hocl2).
           assert (Hocl4 := ForallT_app_r _ _ Hocl2). assumption. }
         ...
         ++ destruct Hocl' as [_ Hocl'].
            ...
            ** eapply Hocl' in Hnil.
               ...
               rewrite rev_app_distr. apply PermutationT_app_comm.
            ** intros Hnil' l''.
               eapply Hocl' in Hnil.
               eapply ex_ir; [ apply Hnil | ].
               symmetry in HP3''. apply (PermutationT_app_tail l'') in HP3''.
               rewrite P_perm. cbn. etransitivity; [ | apply HP3'' ].
               rewrite rev_app_distr, <- app_assoc.
               etransitivity; [ apply PermutationT_app_rot | apply PermutationT_app_rot ].
         ++ inversion Hoclm as [ | ? ? ? HF ]; constructor; try assumption.
            cbn in HP3''.
            apply (PermutationT_ForallT HP3'') in HF.
            list_simpl in HF; apply ForallT_app_r in HF; apply HF.
         ++ list_simpl. rewrite P_perm. cbn.
            cons2app. rewrite <- (app_comm_cons _ _ (ill2ll x2)). apply PermutationT_cons_app.
            list_simpl. etransitivity; [ apply HP1' | ].
            rewrite app_assoc. symmetry. apply PermutationT_cons_app.
            list_simpl. apply PermutationT_app_head.
            symmetry. apply PermutationT_app_rot.
      -- list_simpl in HP4. rewrite app_assoc in HP4. apply PermutationT_cons_app_inv in HP4.
         symmetry in HP4. apply PermutationT_map_inv in HP4 as [l'' Heq' HP].
         list_simpl in Heq'. decomp_map_eq Heq'. subst.
         specialize IHHll1 with (rev l4r) (x1 :: l4l ++ l3 ++ lr) C.
         assert (ForallT oclike (x1 :: l4l ++ l3 ++ lr)) as Hocl'.
         { cbn in HP. cbn in Hocl.
           ...
           constructor; try assumption.
           apply (PermutationT_ForallT HP3') in HF.
           apply ForallT_app_r in HF; apply ForallT_app; try assumption.
           apply ForallT_app_r in Hocl2; assumption. }
         ...
         ++ inversion Hoclm as [ | ? ? ? HF ]; constructor; try assumption.
            apply (PermutationT_ForallT HP3'') in HF.
            list_simpl in HF. apply ForallT_app_l in HF. apply HF.
         ++ list_simpl. rewrite P_perm. cbn.
            cons2app. rewrite <- (app_comm_cons _ _ (ill2ll x1)). apply PermutationT_cons_app.
            list_simpl. etransitivity; [ apply HP2' | ].
            rewrite app_assoc. symmetry. apply PermutationT_cons_app.
            list_simpl. apply PermutationT_app_head. symmetry. apply PermutationT_app_rot.
    * list_simpl in H1. decomp_map_eq H1 eqn:Hx. decomp_map_eq H1. subst.
      apply (f_equal (@rev _)) in H1. list_simpl in H1. subst.
      destruct x; destr_eq Hx.
      -- assert (HP4' := HP4).
         symmetry in HP4'. apply PermutationT_vs_cons_inv in HP4' as [(l4a & l4b) Heq'].
         decomp_elt_eq_app Heq'; subst.
         ++ rewrite map_map in HP3. symmetry in HP3. apply PermutationT_map_inv in HP3 as [l3''' Heq' HP3].
            decomp_map_eq Heq'. subst.
            rewrite <- map_map in HP1, HP2.
            list_simpl in HP4. apply PermutationT_cons_app_inv in HP4.
            apply PermutationT_app_app_inv in HP4 as [[[[l3a l3b] l3c] l3d] [[HP4' HP4''] [HP4''' HP4'''']]].
            apply PermutationT_app_app_inv in HP4'''' as [[[[l3e l3f] l3g] l3h] [[HP4a HP4b] [HP4c HP4d]]].
            assert (PermutationT l2 (map dual (map ill2ll l3') ++ (l3a ++ l3b) ++ ill2ll C :: l3e ++ l3g))
              as HP'.
            { etransitivity; [ apply HP1 | ].
              now apply PermutationT_app_head, PermutationT_app, PermutationT_cons. }
            symmetry in HP4'. apply PermutationT_map_inv in HP4'.
            destruct HP4' as [l4 Heq' HP4'].
            decomp_map_eq Heq'. subst.
            apply (PermutationT_app_head l3b) in HP4d.
            assert (HP'' := PermutationT_trans HP4'' HP4d).
            rewrite map_map in HP''.
            symmetry in HP''. apply PermutationT_map_inv in HP''.
            clear HP4''. destruct HP'' as [l4' Heq' HP4''].
            decomp_map_eq Heq'. subst.
            rewrite <- (map_map _ _ l3b) in HP'.
            rewrite <- (map_map _ _ l3g) in HP'.
            symmetry in HP4c. apply PermutationT_map_inv in HP4c as [l4' Heq' HP4c].
            decomp_map_eq Heq'. subst.
            rewrite <- (map_map _ _ l3h) in HP4b.
            apply (PermutationT_app_head (map dual (map ill2ll l3''))) in HP4b.
            assert (HP'' := PermutationT_trans HP2 HP4b).
            specialize IHHll2 with (rev (x2 :: l3' ++ l3b ++ l3g)) (l3a ++ l3e) C.
            apply (PermutationT_ForallT HP4') in Hocl.
            assert (Hocl1 := ForallT_app_l _ _ Hocl).
            assert (Hocl2 := ForallT_app_r _ _ Hocl).
            specialize IHHll1 with (rev (l3'' ++ l3h)) l3f x1.
            apply (PermutationT_ForallT HP4c) in Hocl2.
            assert (Hocl2' := ForallT_app_l _ _ Hocl2).
            assert (Hocl3 := ForallT_app_r _ _ Hocl2).
            assert (Hocl4 := ForallT_app Hocl1 Hocl2').
            ...
            ** split.
               --- destruct Hocl4 as [Hocl4 _].
                   ...
                   +++ apply PermutationT_rev' in HP3 .
                       apply PermutationT_rev' in HP4''.
                       list_simpl in HP3. list_simpl in HP4''. symmetry in HP4''.
                       rewrite P_perm. list_simpl.
                       etransitivity;
                         [ | apply PermutationT_app_head, PermutationT_cons, HP4''; reflexivity ].
                       rewrite app_assoc. etransitivity; [ apply PermutationT_app_comm | ].
                       rewrite ? app_comm_cons, ? app_assoc.
                       apply PermutationT_app_tail, PermutationT_app_tail.
                       list_simpl.
                       symmetry. etransitivity; [ apply PermutationT_app_tail, HP3 | ].
               ...
               --- intros Hnil lw; destruct lo; [ contradiction Hnil; reflexivity | ].
                   apply (PermutationT_app_head l3a) in HP4c.
                   assert (HP''' := PermutationT_trans HP4' HP4c).
                   symmetry in HP'''. apply PermutationT_vs_cons_inv in HP''' as [(l4l & l4r) Heq'].
                   cbn in Heq'. rewrite app_assoc in Heq'. decomp_elt_eq_app Heq'; subst.
                   +++ rewrite <- Heq' in Hocl4.
                       assert (l4l ++ i :: l3 <> nil) as Hnil' by (intro H; decomp_nil_eq H).
                       ...
                       *** apply PermutationT_rev' in HP3 .
                           ...
                           rewrite P_perm. list_simpl.
                           rewrite ? app_comm_cons, ? app_assoc. apply PermutationT_app_tail.
                           list_simpl. symmetry. etransitivity; [ apply PermutationT_app_tail, HP3 | ].
                           etransitivity;
                             [ apply PermutationT_app_head, PermutationT_cons, HP4''; reflexivity | ].
                           list_simpl. etransitivity; [ apply PermutationT_app_comm | ].
                           rewrite ? app_comm_cons, ? app_assoc. apply PermutationT_app_tail.
                           list_simpl. etransitivity; [ apply PermutationT_app_comm | ].
                           list_simpl. rewrite ? (app_assoc (rev l3g)), ? (app_assoc _ (rev l3')).
                           apply PermutationT_cons_app, PermutationT_app_comm.
                   +++ assert (l3 ++ i :: l4r <> nil) as Hnil'
                       ...
                       *** apply lpam_ilr; assumption.
                       *** apply PermutationT_rev' in HP3 . list_simpl in HP3.
                           apply PermutationT_rev' in HP4''. list_simpl in HP4''.
                           rewrite P_perm. list_simpl.
                           rewrite ? app_comm_cons, ? app_assoc. apply PermutationT_app_tail.
                           list_simpl. symmetry. etransitivity; [ apply PermutationT_app_tail, HP3 | ].
                           etransitivity;
            ...
            ** inversion_clear Hoclm as [ | ? ? ? HF ].
               assert (Hoclm1 := ForallT_app_l _ _ HF).
               assert (Hoclm2 := ForallT_app_r _ _ HF).
               inversion_clear Hoclm2 as [ | ? ? Hoclm3 HF'].
               inversion Hoclm3; constructor; try assumption.
               apply PermutationT_rev' in HP3.
               apply (PermutationT_ForallT HP3) in Hoclm1. list_simpl in Hoclm1.
               assert (Hoclm1' := ForallT_app_l _ _ Hoclm1).
               assert (Hoclm2' := ForallT_app_r _ _ Hoclm1).
               apply PermutationT_rev' in HP4''.
               apply (PermutationT_ForallT HP4'') in HF'. list_simpl in HF'.
               apply ForallT_app_l in HF'. list_simpl. apply ForallT_app; assumption.
            ** cbn. rewrite P_perm, bidual. list_simpl.
               apply PermutationT_cons; [ reflexivity | ].
               etransitivity; [ exact HP'' | apply PermutationT_app_swap_app ].
            ** inversion_clear Hoclm as [ | ? ? ? HF ]; constructor; try assumption.
               ...
               apply (PermutationT_ForallT HP4'') in HF'. list_simpl in HF'.
               apply ForallT_app_r in HF'.
               assert (H4l := ForallT_app_l _ _ HF').
               assert (H4r := ForallT_app_r _ _ HF').
               list_simpl.
               apply ForallT_app; [ exact H4l | ].
               apply ForallT_app; [ exact H4r | ].
               apply PermutationT_rev' in HP3.
               apply (PermutationT_ForallT HP3) in Hoclm1. list_simpl in Hoclm1.
               apply ForallT_app_r in Hoclm1.
               inversion Hoclm2'.
               apply ForallT_app; [ | constructor; [assumption | constructor] ]; assumption.
            ** list_simpl. rewrite P_perm. cbn.
               rewrite app_comm_cons, 2 app_assoc. apply PermutationT_cons_app.
               etransitivity; [ exact HP' | ].
               rewrite ? app_comm_cons, ? app_assoc. apply PermutationT_app_tail. list_simpl.
               symmetry. rewrite ? app_assoc. apply PermutationT_cons_app. list_simpl.
               symmetry. etransitivity; [ apply PermutationT_app_swap_app | ].
               symmetry. apply PermutationT_app_head, PermutationT_app_rot.
         ++ cbn in HP3. rewrite map_map in HP3.
            symmetry in HP3. apply PermutationT_map_inv in HP3 as [l3''' Heq' HP3].
            decomp_map_eq Heq'. subst.
            rewrite <- map_map in HP1, HP2.
            rewrite app_assoc in HP4. apply PermutationT_cons_app_inv in HP4.
            list_simpl in HP4. apply PermutationT_app_app_inv in HP4.
            destruct HP4 as [[[[l3a l3b] l3c] l3d] [[HP4' HP4''] [HP4''' HP4'''']]].
            apply PermutationT_app_app_inv in HP4''''.
            destruct HP4'''' as [[[[l3e l3f] l3g] l3h] [[HP4a HP4b] [HP4c HP4d]]].
            symmetry in HP4'. apply PermutationT_map_inv in HP4' as [l4 Heq' HP4'].
            decomp_map_eq Heq'. subst.
            apply (PermutationT_app_head l3b) in HP4d.
            assert (HP' := PermutationT_trans HP4'' HP4d).
            rewrite map_map in HP'.
            ...
            clear HP4''. destruct HP' as [l4'' Heq' HP4''].
            decomp_map_eq Heq'. subst.
            apply (PermutationT_app_tail (ill2ll C :: l4b)) in HP4a.
            apply (PermutationT_app_head (map dual (map ill2ll l3''))) in HP4a.
            assert (HP' := PermutationT_trans HP2 HP4a).
            rewrite <- (map_map _ _ l3g) in HP'.
            symmetry in HP4c. apply PermutationT_map_inv in HP4c as [l4'' Heq' HP4c].
            decomp_map_eq Heq'. subst.
            rewrite <- (map_map _ _ l3h) in HP4b.
            apply (@PermutationT_cons _ (ill2ll C) _ eq_refl) in HP4b.
            apply (PermutationT_app_head (map dual (map ill2ll l3'')
              ++ (map ill2ll l3e ++ map dual (map ill2ll l3g)))) in HP4b.
            rewrite <- app_assoc in HP4b.
            assert (HP'' := PermutationT_trans HP' HP4b).
            ...
            apply (PermutationT_ForallT HP4') in Hocl.
            apply (PermutationT_app_head l3a) in HP4c.
            apply (PermutationT_ForallT HP4c) in Hocl.
            apply ForallT_app_r in Hocl.
            assert (Hocl' := ForallT_cons _ X1 Hocl).
            ...
            ** apply (snd Hocl') with (ilpam x1 x2 :: rev l3' ++ rev l3b) in Hnil.
               ...
               symmetry. etransitivity; [ apply PermutationT_app_tail, HP3 | ].
               etransitivity;
                 [ apply PermutationT_app_head, PermutationT_cons; [ reflexivity | apply HP4''] | ].
               list_simpl. rewrite ? app_comm_cons, ? app_assoc. apply PermutationT_app_tail.
               list_simpl. rewrite app_assoc. etransitivity; [ apply PermutationT_app_comm | ].
               cons2app. rewrite ? app_assoc. apply PermutationT_app_tail.
               list_simpl. rewrite ? app_assoc. apply PermutationT_cons_app.
            ...
            ** constructor; try assumption.
               apply PermutationT_rev' in HP4''. list_simpl in HP4''.
               apply (PermutationT_ForallT HP4'') in X0.
               rewrite app_assoc in X0. apply ForallT_app_l in X0.
               assert (H4l := ForallT_app_l _ _ X0).
               assert (H4r := ForallT_app_r _ _ X0).
               apply PermutationT_rev' in HP3. list_simpl in HP3.
               apply (PermutationT_ForallT HP3) in Hoclm1.
               apply ForallT_app_l in Hoclm1.
               list_simpl. apply ForallT_app; [ | apply ForallT_app ]; assumption.
            ** list_simpl. rewrite P_perm, bidual. list_simpl.
               ...
               etransitivity; [ apply PermutationT_app_comm | ].
               rewrite ? app_comm_cons, ? app_assoc. apply PermutationT_app_tail. list_simpl.
               symmetry. rewrite ? app_assoc. apply PermutationT_cons_app.
               list_simpl. apply PermutationT_app_head, PermutationT_app_comm.
      -- assert (HP4' := HP4).
         symmetry in HP4'. apply PermutationT_vs_cons_inv in HP4' as [(l4a, l4b) Heq'].
         decomp_elt_eq_app Heq'; subst.
         ++ rewrite map_map in HP3.
            ...
            assert (PermutationT l2 (map dual (map ill2ll l3') ++ (l3a ++ l3b) ++ ill2ll C :: l3e ++ l3g))
              as HP'.
            { etransitivity; [ apply HP1 | ].
              apply PermutationT_app_head, PermutationT_app; auto. }
            symmetry in HP4'. apply PermutationT_map_inv in HP4' as [l4 Heq' HP4'].
            decomp_map_eq Heq'. subst.
            apply (PermutationT_app_head l3b) in HP4d.
            assert (HP'' := PermutationT_trans HP4'' HP4d).
            ...
            ** split.
               --- destruct Hocl4 as [Hocl4 _].
                   ...
                   +++ apply PermutationT_rev' in HP3 . list_simpl in HP3.
                       ...
                       rewrite P_perm. list_simpl.
                       symmetry. etransitivity; [ apply PermutationT_app_tail, HP3 | ].
                       etransitivity; [ apply PermutationT_app_head, PermutationT_cons;
                                          [ reflexivity | apply HP4''] | ].
                       list_simpl. rewrite app_assoc. etransitivity; [ apply PermutationT_app_comm | ].
                       list_simpl. rewrite ? app_assoc. apply PermutationT_cons_app.
                       list_simpl. etransitivity; [ apply PermutationT_app_comm | ].
                       list_simpl. rewrite ? (app_assoc (rev l3g)). apply PermutationT_app_head.
                       rewrite (app_assoc (rev l3')). apply PermutationT_app_comm.
               --- intros Hnil lw. destruct lo; [ contradiction Hnil; reflexivity | ].
                   apply (PermutationT_app_head l3a) in HP4c.
                   assert (HP''' := PermutationT_trans HP4' HP4c).
                   symmetry in HP'''. apply PermutationT_vs_cons_inv in HP''' as [(l4l, l4r) Heq'].
            ...
            ** inversion_clear Hoclm as [ | ? ? ? HF ].
               ...
               assert (Hoclm1' := ForallT_app_l _ _ Hoclm1).
               assert (Hoclm1'' := ForallT_app_r _ _ Hoclm1).
               apply PermutationT_rev' in HP4''.
               apply (PermutationT_ForallT HP4'') in HF'. list_simpl in HF'.
               apply ForallT_app_l in HF'. apply ForallT_app; assumption.
            ** cbn. rewrite P_perm, bidual. list_simpl.
               apply PermutationT_cons; [ reflexivity | ].
               etransitivity; [ exact HP'' | ].
            ...
            ** inversion_clear Hoclm as [ | ? ? ? HF ]; constructor; try assumption.
               ...
               assert (H4l := ForallT_app_l _ _ HF').
               assert (H4r := ForallT_app_r _ _ HF').
               assert (H4r1 := ForallT_app_l _ _ H4r).
               assert (H4r2 := ForallT_app_r _ _ H4r).
               apply ForallT_app; [ apply H4r1 | ].
               apply ForallT_app; [ apply H4r2 | ].
               apply PermutationT_rev' in HP3.
               apply (PermutationT_ForallT HP3) in Hoclm1.
               list_simpl in Hoclm1. apply ForallT_app_r in Hoclm1.
               inversion Hoclm2'.
               list_simpl. apply ForallT_app; [ | constructor; [ constructor | constructor] ]; assumption.
            ** cbn. rewrite P_perm. list_simpl.
               rewrite app_comm_cons, 2 app_assoc. apply PermutationT_cons_app.
               etransitivity; [ exact HP1 | ].
               etransitivity; [ apply PermutationT_app_head, PermutationT_app_head,
                                      PermutationT_cons, HP4a; reflexivity | ].
               list_simpl. rewrite app_assoc. symmetry. apply PermutationT_cons_app.
               rewrite ? map_map. rewrite ? app_assoc. apply PermutationT_app_tail.
               list_simpl. symmetry in HP4'''.
               etransitivity; [ | apply PermutationT_app_head, PermutationT_app_tail, HP4''' ].
               etransitivity; [ | apply PermutationT_app_swap_app ].
               list_simpl. apply PermutationT_app_head.
               etransitivity; [ | apply PermutationT_app_comm ].
         ...
         ++ rewrite map_map in HP3. symmetry in HP3. apply PermutationT_map_inv in HP3 as [l3''' Heq' HP3].
            ...
            clear HP4''. apply PermutationT_map_inv in HP' as [l4'' Heq' HP4''].
            decomp_map_eq Heq'. subst.
            apply (PermutationT_app_tail (ill2ll C :: l4b)) in HP4a.
            apply (PermutationT_app_head (map dual (map ill2ll l3''))) in HP4a.
            assert (HP' := PermutationT_trans HP2 HP4a). rewrite <- (map_map _ _ l3g) in HP'.
            symmetry in HP4c. apply PermutationT_map_inv in HP4c as [l4'' Heq' HP4c].
            decomp_map_eq Heq'. subst.
            rewrite <- (map_map _ _ l3h) in HP4b.
            apply (@PermutationT_cons _ (ill2ll C) _ eq_refl) in HP4b.
            ...
            apply (PermutationT_ForallT HP4c) in Hocl.
            apply ForallT_app_r in Hocl.
            assert (Hocl' := ForallT_cons _ X0 Hocl).
            apply IHHll1 in Hocl'; [ split | | ].
            ...
            ** constructor; try assumption.
               apply PermutationT_rev' in HP4''. list_simpl in HP4''.
               apply (PermutationT_ForallT HP4'') in HF'.
               rewrite app_assoc in HF'. apply ForallT_app_l in HF'.
               assert (H4l := ForallT_app_l _ _ HF').
               assert (H4r := ForallT_app_r _ _ HF').
               apply PermutationT_rev' in HP3. list_simpl in HP3.
               apply (PermutationT_ForallT HP3) in Hoclm1.
               list_simpl in Hoclm1. apply ForallT_app_l in Hoclm1.
               list_simpl. apply ForallT_app; [ assumption | ].
               apply ForallT_app; assumption.
            ** list_simpl. rewrite P_perm, bidual; list_simpl.
               cons2app. rewrite <- (app_comm_cons _ _ (ill2ll x)). apply PermutationT_cons_app.
               etransitivity; [ apply HP'' | ].
               cons2app. rewrite ? app_assoc. apply PermutationT_app_tail.
               list_simpl. etransitivity; [ apply PermutationT_app_comm | ].
               rewrite ? app_comm_cons, ? app_assoc. apply PermutationT_app_tail.
               list_simpl. symmetry. rewrite (app_assoc (map ill2ll l3e) (map dual (map ill2ll l3g))).
               apply PermutationT_cons_app. list_simpl. apply PermutationT_app_head.
               apply PermutationT_app_comm.
      -- assert (HP4' := HP4).
         symmetry in HP4'. apply PermutationT_vs_cons_inv in HP4' as [(l4a, l4b) Heq'].
         ...
         ++ cbn in HP3. rewrite map_map in HP3.
            ...
            apply PermutationT_app_app_inv in HP4 as [[[[l3a l3b] l3c] l3d] [[HP4' HP4''] [HP4''' HP4'''']]].
            apply PermutationT_app_app_inv in HP4'''' as [[[[l3e l3f] l3g] l3h] [[HP4a HP4b] [HP4c HP4d]]].
            apply (@PermutationT_cons _ (ill2ll C) _ eq_refl) in HP4a.
            apply (PermutationT_app HP4''') in HP4a.
            apply (PermutationT_app_head (map dual (map ill2ll l3'))) in HP4a.
            assert (HP' := PermutationT_trans HP1 HP4a).
            symmetry in HP4'. apply PermutationT_map_inv in HP4' as [l4 Heq' HP4'].
            decomp_map_eq Heq'. subst.
            apply (PermutationT_app_head l3b) in HP4d.
            assert (HP'' := PermutationT_trans HP4'' HP4d).
            rewrite map_map in HP''. symmetry in HP''. apply PermutationT_map_inv in HP''.
            clear HP4''. destruct HP'' as [l4' Heq' HP4''].
            decomp_map_eq Heq'. subst.
            rewrite <- (map_map _ _ l3b), <- (map_map _ _ l3g) in HP'.
            symmetry in HP4c. apply PermutationT_map_inv in HP4c as [l4' Heq' HP4c].
            ...
            assert (Hocl2 := ForallT_app_r _ _ Hocl).
            apply (PermutationT_ForallT HP4c) in Hocl2.
            apply ForallT_app_l in Hocl2.
            assert (Hocl3 := ForallT_app Hocl1 Hocl2).
            inversion Hoclm as [ | ? ? ? HF ].
            apply ForallT_app_r in HF.
            inversion HF as [ | ? ? Hoclm' HF']; inversion_clear Hoclm'.
            assert (Hocl4 := ForallT_cons _ X0 Hocl3).
            ...
            ** assert (x1 :: l3a ++ l3e <> nil) as Hnil by intros [=].
               ...
               --- apply (snd Hocl4) with (ilmap x1 x2 :: rev l3'' ++ rev l3h) in Hnil.
                   ...
                   apply PermutationT_rev' in HP4''. list_simpl in HP4''.
                   list_simpl. symmetry. etransitivity; [ apply PermutationT_app_tail, HP3 | ].
                   etransitivity; [ apply PermutationT_app_head, HP4'' | ].
                   list_simpl. rewrite 2 app_assoc. etransitivity; [ apply PermutationT_app_comm | ].
                   rewrite ? app_assoc. apply PermutationT_app_tail.
                   etransitivity; [ apply PermutationT_app_comm | ].
                   rewrite ? app_assoc. apply PermutationT_app_tail.
                   list_simpl. apply PermutationT_app_rot.
               --- intros Hnil' lw.
                   apply (snd Hocl4) with (ilmap x1 x2 :: lw ++ rev l3'' ++ rev l3h) in Hnil.
                   eapply ex_ir; [ apply Hnil | ].
                   rewrite P_perm. cbn.
                   rewrite <- app_assoc, <- app_comm_cons. apply PermutationT_elt.
                   apply PermutationT_rev' in HP3. list_simpl in HP3.
                   apply PermutationT_rev' in HP4''. list_simpl in HP4''.
                   list_simpl. symmetry. etransitivity; [ apply PermutationT_app_tail, HP3 | ].
                   etransitivity; [ apply PermutationT_app_head, PermutationT_app_tail, HP4'' | ].
                   ...
                   rewrite ? app_assoc. apply PermutationT_app_tail.
                   etransitivity; [ apply PermutationT_app_comm | ].
                   rewrite ? app_assoc. apply PermutationT_app_tail, PermutationT_app_tail.
                   list_simpl. apply PermutationT_app_rot.
            ** constructor; [ assumption | ].
               cbn in HP4''.
               apply PermutationT_rev' in HP4''.
               apply (PermutationT_ForallT HP4'') in HF'.
               list_simpl in HF'.
               apply ForallT_app_r in HF' as Hoclm1.
               inversion Hoclm as [ | ? ? ? HF'' ].
               apply ForallT_app_l in HF'' as Hoclm2.
               cbn in HP3.
               apply PermutationT_rev' in HP3.
               apply (PermutationT_ForallT HP3) in Hoclm2.
               list_simpl in Hoclm2.
               apply ForallT_app_r in Hoclm2.
               rewrite rev_app_distr.
               apply ForallT_app; list_simpl; assumption.
            ** cbn. rewrite P_perm, bidual. list_simpl.
               cons2app. rewrite <- (app_comm_cons _ _ (ill2ll x1)). apply PermutationT_cons_app.
               list_simpl. etransitivity; [ exact HP' | ].
               rewrite 2 app_assoc. symmetry. apply PermutationT_cons_app.
               rewrite ? app_assoc. apply PermutationT_app_tail.
               etransitivity; [ | apply PermutationT_app_comm ].
               rewrite ? app_assoc. apply PermutationT_app_tail.
               list_simpl. apply PermutationT_app_rot.
         ++ cbn in HP3. rewrite map_map in HP3.
            ...
            symmetry in HP4'. apply PermutationT_map_inv in HP4' as [l4 Heq' HP4'].
            decomp_map_eq Heq'. subst.
            apply (PermutationT_app_tail l3d) in HP4d.
            assert (HP'' := PermutationT_trans HP4'' HP4d).
            rewrite map_map in HP''.
            symmetry in HP''. apply PermutationT_map_inv in HP''.
            clear HP4''. destruct HP'' as [l4'' Heq' HP4''].
            ...
            decomp_map_eq Heq'. subst.
            rewrite <- (map_map _ _ l3h) in HP4b.
            apply (@PermutationT_cons _ (ill2ll C) _ eq_refl) in HP4''''.
            apply (PermutationT_app HP4b) in HP4''''.
            apply (PermutationT_app_head (map dual (map ill2ll l3''))) in HP4''''.
            assert (HP'' := PermutationT_trans HP2 HP4'''').
            ...
            apply (PermutationT_ForallT HP4c) in Hocl1.
            assert (Hocl3 := ForallT_app_l _ _ Hocl1).
            assert (Hocl4 := ForallT_app_r _ _ Hocl1).
            assert (Hocl5 := ForallT_app Hocl2 Hocl4).
            ...
            ** split.
               --- destruct Hocl5 as [Hocl5 _].
                   ...
                   +++ apply lmap_ilr; try assumption.
                       ...
                       rewrite P_perm. cbn. cons2app. apply PermutationT_app_comm.
                   +++ rewrite P_perm. cbn.
                       apply PermutationT_rev' in HP3. list_simpl in HP3.
                       apply PermutationT_rev' in HP4''. list_simpl in HP4''.
                       list_simpl. symmetry. etransitivity; [ apply PermutationT_app_tail, HP3 | ].
                       etransitivity;
                         [ apply PermutationT_app_head, PermutationT_cons, HP4''; reflexivity | ].
                       list_simpl. rewrite 2 app_assoc. etransitivity; [ apply PermutationT_app_comm | ].
                       list_simpl. rewrite ? (app_assoc (rev l3g)). apply PermutationT_cons_app.
                       rewrite app_assoc. etransitivity; [ apply PermutationT_app_comm | ].
                       list_simpl. apply PermutationT_app_head.
                       etransitivity; [ apply PermutationT_app_comm | list_reflexivity ].
               --- intros Hnil lw. destruct lo; [ contradiction Hnil; reflexivity | ].
                   apply (PermutationT_app_tail l3c) in HP4c.
                   assert (HP''' := PermutationT_trans HP4' HP4c).
                   symmetry in HP'''. apply PermutationT_vs_cons_inv in HP''' as [(l4l & l4r) Heq'].
                   rewrite <- app_assoc in Heq'. decomp_elt_eq_app Heq'; subst.
                   +++ assert (l4l ++ i :: l3 <> nil) as Hnil'
                       ...
                       *** apply lmap_ilr; try assumption.
                           eapply ex_ir; [ apply Hocl5 | ].
                           rewrite P_perm. cbn. symmetry. apply PermutationT_cons_append.
                       *** rewrite P_perm. cbn.
                           apply PermutationT_rev' in HP3; list_simpl in HP3.
                           apply PermutationT_rev' in HP4''. list_simpl in HP4''.
                           list_simpl. symmetry. etransitivity; [ apply PermutationT_app_tail, HP3 | ].
                           etransitivity; [ apply PermutationT_app_head, PermutationT_cons,
                                     PermutationT_app_tail, HP4''; reflexivity | ].
                           list_simpl. rewrite 2 app_assoc. etransitivity; [ apply PermutationT_app_comm | ].
                   ...
                   +++ assert (l3 ++ i :: l4r <> nil) as Hnil'
                       ...
                       *** apply lmap_ilr; [ assumption | ].
                           ...
                           rewrite P_perm. cbn. etransitivity; [ apply PermutationT_app_comm | ].
                           apply PermutationT_app_head.
                           symmetry. etransitivity; [ | apply PermutationT_middle ].
                           list_simpl. reflexivity.
                       *** rewrite P_perm. cbn.
                           apply PermutationT_rev' in HP3. list_simpl in HP3.
                           apply PermutationT_rev' in HP4''. list_simpl in HP4''.
            ...
            ** inversion_clear Hoclm as [ | ? ? ? HF ].
               ...
               --- list_simpl in Hoclm1. apply ForallT_app_l in Hoclm1.
                   ...
                   apply ForallT_rev, Hoclm1.
               --- apply PermutationT_rev' in HP4''.
                   apply (PermutationT_ForallT HP4'') in HF'.
                   list_simpl in HF'.
                   assert (H4l := ForallT_app_l _ _ HF').
                   assert (H4r := ForallT_app_r _ _ HF').
                   apply ForallT_rev in H4l.
                   rewrite rev_involutive in H4l.
                   apply ForallT_app; [ | assumption ].
                   apply ForallT_app_l in H4r.
                   apply ForallT_rev in H4r.
                   rewrite rev_involutive in H4r; assumption.
            ** cbn. rewrite P_perm. list_simpl.
               rewrite app_comm_cons, 2 app_assoc. apply PermutationT_cons_app.
               list_simpl. etransitivity; [ exact HP'' | ].
               rewrite app_assoc. symmetry. apply PermutationT_cons_app.
               list_simpl. rewrite ? app_assoc. apply PermutationT_app_tail.
               list_simpl. etransitivity; [ apply PermutationT_app_comm | ].
               rewrite ? app_assoc.
               apply PermutationT_app_tail, PermutationT_app_tail, PermutationT_app_comm.
            ** inversion_clear Hoclm as [ | ? ? ? HF ].
               assert (Hoclm1 := ForallT_app_l _ _ HF).
               assert (Hoclm2 := ForallT_app_r _ _ HF).
               inversion_clear Hoclm2 as [ | ? ?  Hoclm2' HF' ].
               ...
               assert (H4l := ForallT_app_l _ _ HF').
               assert (H4r := ForallT_app_r _ _ HF').
               apply ForallT_app_r in H4r.
               inversion_clear Hoclm2'; constructor; try assumption.
               list_simpl; apply ForallT_app; try assumption.
               cbn in HP3.
               apply PermutationT_rev' in HP3.
               apply (PermutationT_ForallT HP3) in Hoclm1.
               list_simpl in Hoclm1; apply ForallT_app_r in Hoclm1.
            ...
            ** list_simpl. rewrite P_perm, bidual. list_simpl.
               ...
               rewrite ? map_map. apply PermutationT_app_swap_app.
      -- assert (HP4' := HP4).
         symmetry in HP4'. apply PermutationT_vs_cons_inv in HP4' as [(l4a & l4b) Heq'].
         decomp_elt_eq_app Heq'; subst.
         ++ rewrite map_map in HP3.
            ...
            assert (Hocl3 := ForallT_app Hocl1 Hocl2).
            inversion_clear Hoclm as [ | ? ? ? HF ].
            assert (HF'' := ForallT_app_l _ _ HF).
            apply ForallT_app_r in HF.
            inversion HF as [ | ? ? Hoclm' HF' ]; inversion_clear Hoclm'.
            assert (Hocl4 := ForallT_cons _ X0 Hocl3).
            apply IHHll2 in Hocl4.
            ** assert (x :: l3a ++ l3e <> nil) as Hnil by intros [=].
               ...
               --- apply (snd Hocl4) with (ineg x :: rev l3'' ++ rev l3h) in Hnil.
                   eapply ex_ir; [ apply Hnil | ].
                   rewrite P_perm. cbn. apply PermutationT_elt.
                   cbn in HP3. apply PermutationT_rev' in HP3. list_simpl in HP3.
                   cbn in HP4''. apply PermutationT_rev' in HP4''. list_simpl in HP4''.
                   list_simpl. symmetry.
                   etransitivity; [ apply PermutationT_app_tail, HP3 | ].
                   etransitivity; [ apply PermutationT_app_head, HP4'' | ].
                   symmetry. rewrite app_assoc. etransitivity; [ apply PermutationT_app_comm | ].
                   rewrite ? app_assoc. apply PermutationT_app_tail, PermutationT_app_tail.
                   list_simpl. apply PermutationT_app_swap_app.
               --- intros Hnil' lw.
            ...
            ** constructor; try assumption.
               ...
               apply PermutationT_rev' in HP3.
               apply (PermutationT_ForallT HP3) in HF''.
               list_simpl in HF''. apply ForallT_app_r in HF''.
               rewrite rev_app_distr. apply ForallT_app; list_simpl; assumption.
            ** cbn. rewrite P_perm, bidual. list_simpl.
               cons2app. rewrite <- (app_comm_cons _ _ (ill2ll x)). apply PermutationT_cons_app.
               list_simpl. etransitivity; [ exact HP' | ].
               symmetry. rewrite ? app_assoc. apply PermutationT_cons_app.
               rewrite ? app_assoc. apply PermutationT_app_tail.
               list_simpl. etransitivity; [ | apply PermutationT_app_swap_app ].
               apply PermutationT_app_head, PermutationT_app_rot.
         ++ rewrite map_map in HP3.
            symmetry in HP3. apply PermutationT_map_inv in HP3 as [l3''' Heq' HP3].
            decomp_map_eq Heq'. subst.
            rewrite <- map_map in HP1, HP2.
            ...
            apply PermutationT_map_inv in HP'' as [l4'' Heq' HP4''].
            decomp_map_eq Heq'. subst.
            rewrite <- (map_map _ _ l3g) in HP'.
            symmetry in HP4c. apply PermutationT_map_inv in HP4c as [l4'' Heq' HP4c].
            decomp_map_eq Heq'. subst.
            ...
            ** split.
               --- destruct Hocl5 as [Hocl5 _].
                   ...
                   +++ eapply ex_ir; [ apply Hocl3 | ].
                       rewrite P_perm. cbn.
                       rewrite app_assoc. apply PermutationT_elt.
                       apply PermutationT_rev' in HP3. list_simpl in HP3.
                       apply PermutationT_rev' in HP4''. list_simpl in HP4''.
                       list_simpl. symmetry. etransitivity; [ apply PermutationT_app_tail, HP3 | ].
                       etransitivity; [ apply PermutationT_app_head, HP4'' | ].
                   ...
                   +++ eapply ex_ir; [ apply Hocl5 | ].
                       rewrite P_perm. cbn.
                       etransitivity; [ | apply PermutationT_app_comm ].
                       list_simpl. rewrite 2 app_assoc. symmetry. apply PermutationT_cons_app.
                       list_simpl. apply PermutationT_app_head, PermutationT_app_comm.
               --- intros Hnil lw. destruct lo; [ contradiction Hnil; reflexivity | ].
                   apply (PermutationT_app_tail l3c) in HP4c.
                   assert (HP''' := PermutationT_trans HP4' HP4c).
                   ...
                   +++ assert (l4l ++ i :: l3 <> nil) as Hnil'
                       ...
                       *** eapply ex_ir; [ apply Hnil' | ].
                           ...
                           etransitivity; [ apply PermutationT_app_tail, HP3 | ].
                           etransitivity;
                             [ apply PermutationT_app_head, PermutationT_app_tail, HP4'' | ].
                           list_simpl. apply PermutationT_app_head.
                           symmetry. rewrite app_assoc. etransitivity; [ apply PermutationT_app_comm |  ].
                           list_simpl. apply PermutationT_app_head.
                           etransitivity; [ apply PermutationT_app_comm | list_reflexivity ].
                       ...
                       *** eapply ex_ir; [ apply Hocl5 | ].
                           rewrite P_perm. list_simpl.
                           cons2app. etransitivity; [ apply PermutationT_app_comm | ].
                           rewrite ? app_assoc. apply PermutationT_app_tail.
                           list_simpl. apply PermutationT_app_swap_app.
                   +++ assert (l3 ++ i :: l4r <> nil) as Hnil' by (intro H; decomp_nil_eq H).
                       rewrite Heq' in Hnil'.
                       assert (l3c ++ l3f <> nil) as Hnil''.
                       ...
                       *** eapply ex_ir; [ apply Hocl3 | ].
                           rewrite P_perm. list_simpl.
                           rewrite 3 app_assoc. apply PermutationT_elt.
                           apply PermutationT_rev' in HP3. list_simpl in HP3.
                           apply PermutationT_rev' in HP4''. list_simpl in HP4''.
                           list_simpl. rewrite ? app_assoc. apply PermutationT_app_tail.
                           symmetry. etransitivity; [ apply PermutationT_app_tail, HP3 | ].
            ...
            ** inversion_clear Hoclm as [ | ? ? ? HF ].
               ...
               apply (PermutationT_ForallT HP3) in Hoclm1.
               apply ForallT_rev.
               constructor; [ | apply ForallT_app ].
               --- constructor.
               --- list_simpl in Hoclm1. apply ForallT_app_l in Hoclm1.
                   rewrite <- rev_involutive. apply ForallT_rev, Hoclm1.
               --- apply PermutationT_rev' in HP4''.
                   apply (PermutationT_ForallT HP4'') in HF'. list_simpl in HF'.
                   assert (H4l := ForallT_app_l _ _ HF').
                   assert (H4r := ForallT_app_r _ _ HF').
                   apply ForallT_rev in H4l. rewrite rev_involutive in H4l.
                   apply ForallT_app; [ | assumption ].
                   apply ForallT_app_l in H4r.
                   apply ForallT_rev in H4r.
                   rewrite rev_involutive in H4r. assumption.
            ** cbn. rewrite P_perm. list_simpl.
               ...
               etransitivity; [ exact HP'' | ].
               rewrite ? app_comm_cons, ? app_assoc. apply PermutationT_app_tail.
               list_simpl. symmetry. rewrite app_comm_cons. etransitivity; [ apply PermutationT_app_comm | ].
               rewrite ? app_assoc. apply PermutationT_app_tail.
               list_simpl. apply PermutationT_app_swap_app.
            ** inversion_clear Hoclm as [ | ? ? ? HF ].
               assert (Hoclm1 := ForallT_app_l _ _ HF).
               assert (Hoclm2 := ForallT_app_r _ _ HF).
               inversion_clear Hoclm2 as [ | ? ? Hoclm2' HF' ].
               cbn in HP4''; apply PermutationT_rev' in HP4''.
               apply (PermutationT_ForallT HP4'') in HF'.
               list_simpl in HF'.
               assert (H4l := ForallT_app_l _ _ HF').
               assert (H4r := ForallT_app_r _ _ HF').
               ...
               apply PermutationT_rev' in HP3.
               apply (PermutationT_ForallT HP3) in Hoclm1.
               list_simpl in Hoclm1. apply ForallT_app_r in Hoclm1. apply Hoclm1.
            ** list_simpl. rewrite P_perm. cbn. rewrite bidual.
               apply PermutationT_cons; [ reflexivity | ].
               etransitivity; [ exact HP' | apply PermutationT_app_swap_app ].
- symmetry in HP; apply PCPermutationT_vs_cons_inv in HP as [(l', l'') HP Heq]; cbn in Heq, HP.
  destruct l'; inversion Heq; [ destruct C; inversion H0 | ]; subst.
  + specialize IHHll with (l0 ++ C1 :: nil) lo C2.
    ...
    * destruct Hocl as [IH1 IH2].
      ...
      -- intros Hnil l2.
         ...
         rewrite P_perm. list_simpl. apply PermutationT_app_head, PermutationT_cons_append.
    ...
    * rewrite P_perm in HP. cbn in HP. rewrite app_nil_r in HP.
      cbn. rewrite P_perm. cbn.
      apply PermutationT_cons; [ reflexivity | ].
      list_simpl. apply PermutationT_cons_app. rewrite ! map_rev. assumption.
  + specialize IHHll with (l0 ++ C :: nil) lo N.
    ...
    * destruct Hocl as [IH1 IH2].
      ...
      -- intros Hnil l2.
         ...
         eapply ex_ir; [ eassumption | ].
         rewrite P_perm. list_simpl.
         apply PermutationT_app_head, PermutationT_cons_append.
    * inversion_clear Hoclm as [ | ? ? Hoclm' ].
      inversion_clear Hoclm'.
      constructor; try assumption.
      -- constructor.
      -- apply ForallT_app; try assumption.
         constructor; [ | constructor ]; assumption.
    * rewrite P_perm in HP. cbn in HP. rewrite app_nil_r in HP.
      cbn. rewrite P_perm.
      apply PermutationT_cons; [ reflexivity | ].
  ...
  + specialize IHHll with (C1 :: l0) lo C2.
    ...
    * cbn in HP. rewrite P_perm in HP. cbn in HP.
      cbn. rewrite P_perm. cbn.
      rewrite app_comm_cons, app_assoc. apply PermutationT_cons_app.
  ...
  + specialize IHHll with (C :: l0) lo N.
    ...
    * cbn in HP. rewrite P_perm in HP. cbn in HP.
      ...
      now apply PermutationT_cons.
  + decomp_elt_eq_app H1; subst;
      [ decomp_map_eq H1 eqn:Hx
      | list_simpl in H1; decomp_map_eq H1 eqn:Hx; decomp_map_eq H1 ];
      destruct x; destr_eq Hx; subst;
      [ exfalso; apply ForallT_app_r in Hocl; inversion Hocl as [ | ? ? Hocl'];
        inversion Hocl' .. | ].
    apply (f_equal (@rev _)) in H1. list_simpl in H1. subst.
    apply (PEPermutationT_cons _ (eq_refl (dual (ill2ll x1)))) in HP.
    apply (PEPermutationT_cons _ (eq_refl (dual (ill2ll x2)))) in HP.
    apply PEPermutation_PCPermutationT in HP. unfold id in HP. rewrite 2 app_comm_cons in HP.
    assert (HP' := PCPermutationT_trans _ _ _ _ HP (PCPermutationT_app_comm _ _ _)).
    ...
    * inversion Hoclm as [ | ? ? ? HF ]; constructor; try assumption.
      assert (Hl := ForallT_app_l _ _ HF).
      assert (Hr := ForallT_app_r _ _ HF).
      apply ForallT_app; [ assumption | ].
      inversion Hr as [ | ? ? Hr' ].
      inversion Hr'.
      constructor; [ | constructor ]; assumption.
- symmetry in HP. apply PCPermutationT_vs_cons_inv in HP as [(l', l'') HP Heq].
  ...
  + symmetry in H1. decomp_elt_eq_app H1; subst;
      ...
      destruct x; destr_eq Hx; subst.
    * exfalso.
      apply ForallT_app_r in Hocl. inversion Hocl as [ | ? ?  Hocl' ]. inversion Hocl'.
    * apply (f_equal (@rev _)) in H1. list_simpl in H1. subst.
      split; intros; list_simpl; apply zero_ilr.
- symmetry in HP. apply PCPermutationT_vs_cons_inv in HP as [([|], l'') HP Heq];
    inversion Heq; [ destruct C; inversion H0 | ]; subst.
  + apply (PEPermutationT_cons _ (eq_refl (ill2ll C1))) in HP.
  ...
  + symmetry in H1. decomp_elt_eq_app H1; subst;
      ...
      | list_simpl in H1; decomp_map_eq H1 eqn:Hx; decomp_map_eq H1 ];
      destruct x; destr_eq Hx; subst.
    * apply (PEPermutationT_cons _ (eq_refl (ill2ll x1))) in HP.
      apply PEPermutation_PCPermutationT in HP. unfold id in HP. rewrite app_comm_cons in HP.
      assert (HP' := PCPermutationT_trans _ _ _ _ HP (PCPermutationT_app_comm _ _ _)).
...
- symmetry in HP; apply PCPermutationT_vs_cons_inv in HP as [(l', l'') HP Heq].
  destruct l'; inversion Heq; [ destruct C; inversion H0 | ]; subst.
  + apply (PEPermutationT_cons _ (eq_refl (ill2ll C2))) in HP.
    apply PEPermutation_PCPermutationT in HP. unfold id in HP. rewrite app_comm_cons in HP.
    assert (HP' := PCPermutationT_trans _ _ _ _ HP (PCPermutationT_app_comm _ _ _)).
...
- symmetry in HP; apply PCPermutationT_vs_cons_inv in HP as [(l', l'') HP Heq].
  destruct l'; inversion Heq; [ destruct C; inversion H0 | ]; subst.
  + assert (HP1 := HP).
    apply (PEPermutationT_cons _ (eq_refl (ill2ll C1))) in HP1.
    apply PEPermutation_PCPermutationT in HP1. unfold id in HP1. rewrite app_comm_cons in HP1.
    assert (HP1' := PCPermutationT_trans _ _ _ _ HP1 (PCPermutationT_app_comm _ _ _)).
    ...
    * destruct HP1' as [IH1 IH2].
      apply (PEPermutationT_cons _ (eq_refl (ill2ll C2))) in HP.
      apply PEPermutation_PCPermutationT in HP. unfold id in HP. rewrite app_comm_cons in HP.
      assert (HP2' := PCPermutationT_trans _ _ _ _ HP (PCPermutationT_app_comm _ _ _)).
  ...
  + symmetry in H1. decomp_elt_eq_app H1; subst;
    ...
    * apply (f_equal (@rev _)) in H1. list_simpl in H1. subst.
      assert (HP1 := HP).
      apply (PEPermutationT_cons _ (eq_refl (dual (ill2ll x1)))) in HP1.
      apply PEPermutation_PCPermutationT in HP1. unfold id in HP1. rewrite app_comm_cons in HP1.
      assert (HP1' := PCPermutationT_trans _ _ _ _ HP1 (PCPermutationT_app_comm _ _ _)).
...
- symmetry in HP. apply PCPermutationT_vs_cons_inv in HP as [(l', l'') HP Heq].
  destruct l'; inversion Heq; [ destruct C; inversion H0 | ]; subst.
  + symmetry in HP. apply PEPermutationT_map_inv in HP as [l'' Heq' HPE]. list_simpl in Heq'.
    remember (map ill2ll lo) as lo'.
    remember (map dual (map ill2ll (rev l0))) as l0'.
    decomp_map_eq Heq' eqn:HH. destruct HH as [Heq1 Heq2].
    ...
    * split.
      -- list_simpl in Heq2.
         ...
         ++ etransitivity.
            ** apply PEPermutation_PCPermutationT.
               list_simpl in HPE. apply (PEPermutationT_map _ wn) in HPE.
               apply PEPermutationT_cons; [reflexivity | eassumption].
  ...
  + exfalso.
    symmetry in H1. decomp_elt_eq_app H1; subst;
      ...
      | list_simpl in H1; decomp_map_eq H1 eqn:Hx; decomp_map_eq H1 ];
      destruct x; destr_eq Hx; subst.
    symmetry in HP. apply PEPermutationT_map_inv in HP as [l'' Heq' _].
    decomp_map_eq Heq' eqn:HH. destruct HH as [_ [HH _]].
...
- symmetry in HP. apply PCPermutationT_vs_cons_inv in HP as [(l', l'') HP Heq].
  ...
  symmetry in H1. decomp_elt_eq_app H1; subst;
    [ decomp_map_eq H1 eqn:Hx
    | list_simpl in H1; decomp_map_eq H1 eqn:Hx; decomp_map_eq H1 ];
    destruct x; destr_eq Hx; cbn in Hx, H1; subst.
  apply (f_equal (@rev _)) in H1. list_simpl in H1. subst.
  apply (PEPermutationT_cons _ (eq_refl (dual (ill2ll x)))) in HP.
  apply PEPermutation_PCPermutationT in HP. unfold id in HP. rewrite app_comm_cons in HP.
  assert (HP' := PCPermutationT_trans _ _ _ _ HP (PCPermutationT_app_comm _ _ _)).
...
- symmetry in HP. apply PCPermutationT_vs_cons_inv in HP as [(l', l'') HP Heq].
  ...
  symmetry in H1. decomp_elt_eq_app H1; subst;
    [ decomp_map_eq H1 eqn:Hx
    | list_simpl in H1; decomp_map_eq H1 eqn:Hx; decomp_map_eq H1 ];
    destruct x; destr_eq Hx; cbn in Hx; cbn in H1; subst.
  apply (f_equal (@rev _)) in H1. list_simpl in H1. subst.
  apply PEPermutation_PCPermutationT in HP. unfold id in HP. rewrite app_comm_cons in HP.
  assert (HP' := PCPermutationT_trans _ _ _ _ HP (PCPermutationT_app_comm _ _ _)).
  specialize IHHll with (rev l'' ++ rev l1) lo C.
...
- symmetry in HP. apply PCPermutationT_vs_cons_inv in HP as [([|], l'') HP Heq];
  ...
  symmetry in H1. decomp_elt_eq_app H1; subst;
    ...
    destruct x; destr_eq Hx; cbn in Hx; cbn in H1; subst.
  apply (f_equal (@rev _)) in H1. list_simpl in H1. subst.
  assert (PCPermutationT (ipperm P) (wn (dual (ill2ll x)) :: wn (dual (ill2ll x)) :: l)
  ...
  + destruct HP' as [IH1 IH2].
    ...
    * eapply ex_ir; [ apply co_ilr | ].
      -- rewrite app_nil_l.
         ...
         rewrite P_perm. cbn. symmetry. apply PermutationT_cons_app, PermutationT_middle.
      -- rewrite P_perm. cbn. apply PermutationT_middle.
    * intros Hnil l3.
      ...
      -- rewrite P_perm. cbn. rewrite app_comm_cons, app_assoc.
         apply PermutationT_app_tail, PermutationT_middle.
  + inversion Hoclm as [ | ? ? ? HF ]. constructor; [ assumption | ].
    assert (Hl := ForallT_app_l _ _ HF).
    assert (Hr := ForallT_app_r _ _ HF).
    apply ForallT_app; [ assumption | ].
...
- destruct lo.
  ...
  + exfalso.
    ...
    apply PCPermutationT_vs_elt_inv in HP as [(l1,l2) _ Heq].
    ...
    * apply (f_equal (@rev _)) in Heq''.
      ...
      decomp_map_eq Heq'' eqn:HH. destruct HH as [Heq1 [Heq2 Heq3]].
      apply (fst (fst (fst (Hgax a))) i); [ | assumption ].
      apply (f_equal dual) in Heq2. rewrite bidual in Heq2.
      rewrite Heq''. list_simpl. rewrite Heq2. apply inT_elt.
Qed.

Lemma ll_to_ill_oclpam_axfree P (P_perm : ipperm P = true) (P_axfree : no_iax P) l C :
  ForallT oclpam (C :: l) -> ll (i2pfrag P) (ill2ll C :: rev (map dual (map ill2ll l))) -> ill P l C.
Proof.
intros Hoclm pi.
apply cut_admissible_axfree in pi; [ | assumption ].
apply (stronger_pfrag _ _ (cutrm_i2pfrag P)) in pi.
