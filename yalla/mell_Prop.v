...
Proof.
intros pi; induction pi; try (now ((try inversion IHpi); do 2 constructor)); rewrite ? map_app.
...
- symmetry in Heql0; decomp_map Heql0; subst; symmetry in Heql0.
  ...
  apply (Permutation_Type_map wn), Permutation_Type_Permutation in p.
  eapply (@ex_r (l0 ++ map wn l'' ++ l5)).
  + apply IHpi; list_simpl; rewrite mell2ll_map_wn; reflexivity.
  + symmetry; apply Permutation_app_head, Permutation_app_tail; assumption.
- symmetry in Heql0; decomp_map Heql0; subst.
...
intros H%Hi; destruct H; assumption.
Qed.
...
rewrite map_app.
eapply ll_cut.cut_r_axfree; [ intros a; destruct a | rewrite mell2ll_dual | ]; eassumption.
Qed.
