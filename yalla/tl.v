...
Proof. split; intro; [ apply tsub_id | apply tsub_trans ]. Qed.
...
Proof. intros x y [= Heq]. exact (TAtom2PreIAtom_inj _ _ Heq). Qed.
...
Fixpoint tl2ill_inv (A : iformula) :=
match A with
...
split; [ split | ].
- intro A. apply BoolOrder.le_trans with (tpcut Q A); [ apply Hc1 | apply Hc2 ].
- intro a. destruct (Ha1 a) as [b ->], (Ha2 b) as [c ->].
  exists c. reflexivity.
- apply BoolOrder.le_trans with (tpperm Q); assumption.
Qed.
...
Proof. repeat split; try reflexivity. intro a. exists a. reflexivity. Qed.
...
Proof. intros Hcut A. cbn. destruct (tl2ill_dec A) as [[[B ->]|_]|_]; [ apply Hcut | | ]; reflexivity. Qed.

Lemma cutrm_t2ipfrag_le P : le_ipfrag (cutrm_ipfrag (t2ipfrag P)) (t2ipfrag (cutrm_tpfrag P)).
Proof. split; split; [ intro a; exists a | ]; reflexivity. Qed.
...
intros pi. induction pi;
  ...
  try rewrite ? map_app in piS, piE;
  try rewrite ? map_app in piS1, piE1;
...
- rewrite <- ipcut_t2ipfrag in f.
  exact (cut_ir (tl2ill A) f (piS1 _ eq_refl) (piS2 _ HeqC)).
- rewrite <- ipcut_t2ipfrag in f.
  exact (cut_ir (tl2ill A) f (piS1 _ eq_refl) (piE2 HeqC)).
- assert (pi := @gax_ir _ (t2ipfrag P) a). cbn in pi. rewrite HeqC in pi. assumption.
- assert (pi := @gax_ir _ (t2ipfrag P) a). cbn in pi. rewrite HeqC in pi. assumption.
Qed.
...
induction pi;
  ...
  try (decomp_map_eq Heql eqn:Hx; destruct x; discriminate Hx);
  try discriminate HeqN;
  try (decomp_map_eq Heql eqn:Hx; subst; destruct x; destr_eq Hx; subst;
       constructor; apply IHpi; list_reflexivity);
  try (destruct A''; destr_eq HeqA; subst; constructor; apply IHpi; reflexivity).
- decomp_map_eq Heql. destruct Heq as [Heq ->]. subst l''.
...
- remember (map ioc lw') as l0. decomp_map_eq Heql eqn:Heq. subst.
...
- destruct l''; destr_eq Heql.
  ...
  apply one_trr.
- decomp_map_eq Heql. destruct A''; destr_eq HeqA; subst.
  apply tens_trr; [ apply IHpi1 | apply IHpi2 ]; reflexivity.
- decomp_map_eq Heql eqn:Hx. destruct Hx as [Hx ->]. destruct x; destr_eq Hx. subst.
  apply neg_tlr, IHpi; reflexivity.
- decomp_map_eq Heql eqn:Hx. destruct x; destr_eq Hx. subst.
  apply plus_tlr; [ apply IHpi1 | apply IHpi2 ]; list_reflexivity.
- decomp_map_eq Heql eqn:Hx. destruct x; destr_eq Hx. subst.
  apply plus_tlr; [ apply IHpi1 | apply IHpi2 ]; list_reflexivity.
...
- cbn in f. destruct (tl2ill_dec A) as [[[B ->]|]|]; [ | discriminate f | discriminate f].
  ...
  + apply IHpi2; list_reflexivity.
- cbn in Heql, HeqA.
  destruct (snd (projT2 (tpgax P) a)) eqn:Heq.
  + apply tl2ill_inj in HeqA as ->.
    rewrite <- Heq.
    apply map_injective in Heql as <-; [ | apply tl2ill_inj ].
    apply gax_tr.
  + contradiction (N_not_tl2ill _ HeqA).
- cbn in Heql, HeqN.
  destruct (snd (projT2 (tpgax P) a)) eqn:Heq.
  + symmetry in HeqN. contradiction (N_not_tl2ill _ HeqN).
  + apply map_injective in Heql as <-; [ | apply tl2ill_inj ].
    rewrite <- Heq. apply gax_tr.
Qed.
...
- intros l C HP.
  ...
  { apply PCPermutationT_vs_cons_inv in HP.
      ...
      + rewrite app_nil_r in HP. apply PEPermutationT_rev in HP. rewrite 2 rev_involutive in HP.
        ...
        * clear Heql0. revert l0 Hin HP. induction l; intros l0 Hin HP; cbn in HP.
          ...
          -- symmetry in HP.
             ...
             rewrite map_map in Heq. decomp_map_eq Heq eqn:Hx. subst. symmetry in Hx; list_simpl.
    ...
    - exfalso.
      ...
      rewrite map_map in Heq'. decomp_map_eq Heq'. subst. destruct Heq as [_ [Heq _]].
...
- intros [(l1,l2) Heq]%inT_split.
  decomp_map_eq Heq eqn:Hx. symmetry in Hx. exact (N_not_tl2ill _ Hx).
Qed.
