...
Lemma cut_oc_comm_left (P_cutfree : ipcut P = false) n A C l1 l2 :
...
Proof.
induction n; intros IH l0 pi1 Hs;
...
- list_simpl; rewrite app_assoc; apply tens_ilr.
  list_simpl; rewrite 2 app_comm_cons, (app_assoc l0).
  cbn in Hs; refine (IHn _ _ Hl _); [ | lia ].
  intros; refine (IH _ pi0 _); lia.
...
Lemma substitution_ioc (P_cutfree : ipcut P = false) A :
...
- destruct l' ; inversion Heq; subst.
  ...
  + list_simpl.
    apply gen_ilr, IHpi2; reflexivity.
- apply lmap_irr.
...
- elt_vs_app_flat_map_cst_inv Heq.
  ...
  + symmetry in Heq2; apply app_eq_nil in Heq2 as [Heq Heq2]; subst.
...
- (* ex_oc_ir *)
  ...
  + dichot_elt_app_inf_exec Heql1 ; subst.
    * symmetry in Heql0; decomp_map_inf Heql0; subst; cbn in HP, pi1.
      ...
      intros lw pi0 Hs'.
      list_simpl; rewrite (app_assoc (map ioc l4)), (app_assoc _ (map ioc l7)),
                          <- (app_assoc (map ioc l4)), <- 2 map_app;
...
- (* wk_ilr *)
  ...
  + list_simpl.
    ...
    refine (IHsize _ _ _ _ _ pi1 Hl _ _); lia.
  + eapply (@cut_oc_comm_left P_cutfree (ipsize pi1)); [ | reflexivity ].
    intros lw Hs pi; apply wk_list_ilr; assumption.
  + rewrite 2 app_assoc.
    apply wk_ilr.
    revert Hl IHsize; list_simpl; intros Hl IHsize.
    refine (IHsize _ _ _ _ _ pi1 Hl _ _); lia.
- (* co_ilr *)
  ...
  + eapply (@cut_oc_comm_left P_cutfree (ipsize pi1)); [ | reflexivity ].
    ...
    refine (substitution_ioc _ _ _ _ _ _ _ _); list_simpl; try eassumption; [ | reflexivity ].
