...
Lemma app_vs_flat_map {T} {f : nat -> T -> T} : forall (A : T) L l1 l2,
  l1 ++ l2 = flat_map (fun p => (f (fst p) A) :: (snd p)) L ->
      {' (L1, L2, n, l3, l4) | l4 <> nil
          /\ L = L1 ++ (n , l3 ++ l4) :: L2
          /\ l1 = flat_map (fun p => (f (fst p) A) :: (snd p)) (L1 ++ (n, l3) :: nil)
          /\ l2 = l4 ++ flat_map (fun p => (f (fst p) A) :: (snd p)) L2 }
       + {' (L1, L2) | L = L1 ++ L2
          /\ l1 = flat_map (fun p => (f (fst p) A) :: (snd p)) L1
          /\ l2 = flat_map (fun p => (f (fst p) A) :: (snd p)) L2 }.
  ...
  + destruct l1.
    ...
    * inversion Heq0 ; subst.
      ...
      -- right.
         ...
         exists ((n,l1) :: nil , L) ; split ; [ | split ] ; simpl ; try rewrite app_nil_r...
         rewrite H1...
      -- left.
         destruct a; simpl in H1.
         exists (nil, L, n , l1, t :: l) ; split ; [ | split ; [ | split ]] ;
...
Ltac app_vs_flat_map_inv H :=
  ...
  | ?lh ++ ?lr = flat_map (fun p => (?f (fst p) ?A) :: (snd p)) ?L =>
...
Lemma app_vs_app_flat_map {T} {f : nat -> T -> T} : forall (A : T) l0 L l1 l2,
  l1 ++ l2 = l0 ++ flat_map (fun p => (f (fst p) A) :: (snd p)) L ->
      { l' | l' <> nil
          /\ l0 = l1 ++ l'
          /\ l2 = l' ++ flat_map (fun p => (f (fst p) A) :: (snd p)) L }
      + {' (L1, L2, n, l3, l4) | l4 <> nil
          /\ L = L1 ++ (n , l3 ++ l4) :: L2
          /\ l1 = l0 ++ flat_map (fun p => (f (fst p) A) :: (snd p)) (L1 ++ (n, l3) :: nil)
          /\ l2 = l4 ++ flat_map (fun p => (f (fst p) A) :: (snd p)) L2 }
      + {' (L1, L2) | L = L1 ++ L2
          /\ l1 = l0 ++ flat_map (fun p => (f (fst p) A) :: (snd p)) L1
          /\ l2 = flat_map (fun p => (f (fst p) A) :: (snd p)) L2 }.
Proof with try assumption ; try reflexivity.
...
Ltac app_vs_app_flat_map_inv H :=
  match type of H with
  | ?lh ++ ?lr = ?l0 ++ flat_map (fun p => (?f (fst p) ?A) :: (snd p)) ?L =>
...
Lemma elt_vs_app_flat_map {T} {f : nat -> T -> T} : forall (A : T) l0 L l1 l2 B,
  l1 ++ B :: l2 = l0 ++ flat_map (fun p => f (fst p) A :: snd p) L ->
      { l' | l0 = l1 ++ B :: l'
          /\ l2 = l' ++ flat_map (fun p => f (fst p) A :: snd p) L }
    + {' (L1, L2, n, l1',l2') | L = L1 ++ (n , l1' ++ B :: l2') :: L2
          /\ l1 = l0 ++ flat_map (fun p => f (fst p) A :: snd p) L1 ++ (f n A :: l1')
          /\ l2 = l2' ++ flat_map (fun p => f (fst p) A :: snd p) L2 }
    + {' (L1 , L2, n , l) | B = f n A
          /\ L = L1 ++ (n , l) :: L2
          /\ l1 = l0 ++ flat_map (fun p => f (fst p) A :: snd p) L1
          /\ l2 = l ++ flat_map (fun p => f (fst p) A :: snd p) L2 }.
Proof with try assumption ; try reflexivity.
...
Lemma map_f_flat_map {T1 T2} {g : nat -> T2 -> T2} : forall A (f : T1 -> T2) lw' lw l L ,
  map f lw = l ++ flat_map (fun p => g (fst p) (f A) :: (snd p)) L -> { Lw | l ++ flat_map (fun p => app (map f lw') (snd p)) L = map f Lw }.
Proof.
...
Lemma perm_Type_app_flat_map {T} {f : nat -> T -> T}: forall (A : T) lw0 L lw l,
  Permutation_Type lw (l ++ flat_map (fun p => f (fst p) A :: snd p) L) ->
{' (L'  , lw') : _  & prod (L <> nil -> L' <> nil)
      (prod (lw = lw' ++ flat_map (fun p => f (fst p) A :: snd p) L')
...
Lemma perm_Type_f_app_flat_map {T1 T2} {g : nat -> T2 -> T2} : forall A (f : T1 -> T2) lw0 L lw lw' l,
  Permutation_Type lw lw' -> map f lw' = l ++ flat_map (fun p => g (fst p) (f A) :: snd p) L ->
{' (L' , lw') : _ & prod (L <> nil -> L' <> nil)
      (prod (map f lw = lw' ++ flat_map (fun p => g (fst p) (f A) :: snd p) L')
...
Lemma perm_flat_map_cons_flat_map_app {T1 T2} {g : nat -> T2 -> T2} :
forall A (f : T1 -> T2), injective f -> forall l0 l1 l2 lp1 lp2 l L,
  Permutation_Type lp1 lp2 ->
  l1 ++ map f lp2 ++ l2 = l ++ flat_map (fun p => g (fst p) (f A) :: snd p) L ->
{' (l1',l2',l3',l4',l',L') : _ &  prod (l1 ++ map f lp1 ++ l2 = l' ++ flat_map (fun p => g (fst p) (f A) :: snd p) L')
...
Lemma cperm_Type_app_flat_map {T} {f : nat -> T -> T}: forall (A : T) lw0 L lw l,
  CPermutation_Type lw (l ++ flat_map (fun p => f (fst p) A :: (snd p)) L) ->
{'(L' , lw') : _ & prod (L <> nil -> L' <> nil)
      (prod (lw = lw' ++ flat_map (fun p => f (fst p) A :: (snd p)) L')
